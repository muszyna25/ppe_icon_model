...
module radiation_overlap
  ...
  public :: calc_region_fractions, &
       &  calc_overlap_matrices
...
contains
  ...
  pure function calc_alpha_overlap_matrix(nreg, op, op_inhom, &
    ...
    real(jprb), intent(in) :: frac_upper, frac_lower
    ...
    pair_cloud_cover = op*max(frac_upper,frac_lower) &
           &  + (1.0_jprb - op) &
           &  * (frac_upper+frac_lower-frac_upper*frac_lower)

    ! Clear in both layers
    overlap_matrix(1,1) = 1.0_jprb - pair_cloud_cover
    if (nreg == 2) then
      ! Clear in upper layer, cloudy in lower layer
      overlap_matrix(1,2) = pair_cloud_cover - frac_upper
      ! Clear in lower layer, cloudy in upper layer
      overlap_matrix(2,1) = pair_cloud_cover - frac_lower
      ! Cloudy in both layers
      overlap_matrix(2,2) = frac_upper + frac_lower - pair_cloud_cover
    else
      ! Clear in upper layer, cloudy in lower layer
      overlap_matrix(1,2) = 0.5_jprb * (pair_cloud_cover - frac_upper)
      overlap_matrix(1,3) = overlap_matrix(1,2)
      ! Clear in lower layer, cloudy in upper layer
      overlap_matrix(2,1) = 0.5_jprb * (pair_cloud_cover - frac_lower)
      overlap_matrix(3,1) = overlap_matrix(2,1)
      ! Cloudy in both layers
      cloud_unit = 0.25_jprb * (frac_upper + frac_lower - pair_cloud_cover)
  ...
  subroutine calc_overlap_matrices(nlev,nreg,istartcol,iendcol, &
       &     cloud_fraction, overlap_param, u_matrix, v_matrix, decorrelation_scaling, &
       &     cloud_fraction_threshold, cloud_cover, use_beta_overlap)

    use parkind1,     only : jprb
    use ecradhook,      only : lhook, dr_hook

    ! Number of columns, levels and regions
    integer,  intent(in) :: nlev, nreg

    ! Range of columns to proces (also outer dimensions of u_matrix
    ! and v_matrix)
    ...
    ! The "beta" overlap parameter of Shonk et al. (2010)
    ...
    do jcol = istartcol, iendcol
      ...
      do jlev = 1,nlev+1
        ...
        if (jlev > nlev) then
          ...
          frac_lower = calc_region_fractions(nreg,cloud_fraction(jcol,jlev))
        end if
        ...
        if (use_beta_overlap_param) then
          overlap_matrix = calc_beta_overlap_matrix(nreg, op, frac_upper, frac_lower, &
               &                       frac_threshold)
        else
          overlap_matrix = calc_alpha_overlap_matrix(nreg, op(1), op(2), &
