...
!    real(jprb), intent(in), dimension(istartcol:iendcol,nlev,config%n_g_sw) :: &
    ...
    real(jprb) :: od_scaling(2:nregions)
    ...
    real(jprb), dimension(nregions) :: frac
    ...
    ! Compute wavelength-independent overlap matrices U and V
    call calc_overlap_matrices(nlev,nregions,istartcol,iendcol, &
         &  cloud%fraction, cloud%overlap_param, &
         &  U, V, decorrelation_scaling=config%cloud_inhom_decorr_scaling, &
    ...
    do jcol = istartcol, iendcol
      ...
      do jlev = 1,nlev ! Start at top-of-atmosphere
        ...
        if (is_clear_sky_layer(jlev)) then
          ...
          call optical_depth_scaling(nregions, cloud%fractional_std(jcol,jlev), &
      ...
      do jlev = nlev,1,-1
        ...
        if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev-1)) then
          ...
          ! = matmul(transpose(V), diag(total_albedo_below)).
          do jreg = 1,nregions
            do jreg2 = 1,nregions
              total_albedo(:,jreg,jlev) &
                   &  = total_albedo(:,jreg,jlev) &
                   &  + total_albedo_below(:,jreg2) &
                   &  * V(jreg2,jreg,jlev,jcol)
      ...
      frac = calc_region_fractions(nregions,cloud%fraction(jcol,1))
      ...
      do jreg = 1,nregions
        direct_dn(:,jreg) = incoming_sw(:,jcol)*frac(jreg)
      end do
      ...
      if (config%do_save_spectral_flux) then
        ...
        call indexed_sum(sum(direct_dn,2), &
             &           config%i_spec_from_reordered_g_sw, &
             &           flux%sw_dn_direct_band(:,jcol,1))
        flux%sw_dn_direct_band(:,jcol,1) = &
             &  mu0 * flux%sw_dn_direct_band(:,jcol,1)
        ...
        if (config%do_clear) then
          ...
          flux%sw_dn_direct_clear_band(:,jcol,1) &
               &  = mu0 * flux%sw_dn_direct_clear_band(:,jcol,1)
      ...
      do jlev = 1,nlev
        ...
        if (.not. (is_clear_sky_layer(jlev) &
          ...
          flux_dn_below = singlemat_x_vec(ng,ng,nregions,V(:,:,jlev+1,jcol), &
               &  flux_dn)
          direct_dn_below = singlemat_x_vec(ng,ng,nregions,V(:,:,jlev+1,jcol), &
               &  direct_dn)
        ...
        if (config%do_save_spectral_flux) then
          ...
          flux%sw_dn_direct_band(:,jcol,jlev+1) = &
               &  mu0 * flux%sw_dn_direct_band(:,jcol,jlev+1)
          ...
          if (config%do_clear) then
            ...
            flux%sw_dn_direct_clear_band(:,jcol,jlev+1) = &
                 &  mu0 * flux%sw_dn_direct_clear_band(:,jcol,jlev+1)
