  ...
  subroutine adding_ica_sw(ncol, nlev, incoming_toa, &
    ...
    do jlev = nlev,1,-1
      ...
      do jcol = 1,ncol
        ...
        !T_over_denom = T(jcol,jlev)*inv_denominator(jcol,jlev)
        ! Shonk & Hogan (2008) Eq 9, Petty (2006) Eq 13.81:
        albedo(jcol,jlev) = R(jcol,jlev) + albedo(jcol,jlev+1) &
             &                     * T(jcol,jlev) * T(jcol,jlev)*inv_denominator(jcol,jlev)
        ...
        source(jcol,jlev) = Sup(jcol,jlev)*flux_dn_direct(jcol,jlev) &
             ...
             &  * T(jcol,jlev)*inv_denominator(jcol,jlev)
      end do
    end do
