...
!         &   * iwp_kg_m2 / cloud%re_ice
    ...
    ! Values typical for 0.55 micron wavelength, effective radius 10
    ! microns
    ssa_sw_cloud = 0.999999_jprb
    g_sw_cloud   = 0.86_jprb
!    g_sw_cloud   = 0.0_jprb

    print *, 'OD = ', sum(od_sw_cloud(1,:,istartcol)*cloud%fraction(istartcol,:))

    ! In-place delta-Eddington scaling
    call delta_eddington(od_sw_cloud, ssa_sw_cloud, g_sw_cloud)

    print *, 'OD scaled = ', sum(od_sw_cloud(1,:,istartcol)*cloud%fraction(istartcol,:))
