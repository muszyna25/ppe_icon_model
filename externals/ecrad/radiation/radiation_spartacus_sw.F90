...
#include "radiation_optical_depth_scaling.h"
  ...
  subroutine solver_spartacus_sw(nlev,istartcol,iendcol, &
    ...
    use radiation_config, only         : config_type, IPdfShapeGamma, &
         &  IEncroachmentMinimum, IEncroachmentMaximum, IEncroachmentComputed
    use radiation_single_level, only   : single_level_type
    use radiation_thermodynamics, only : thermodynamics_type
    use radiation_cloud, only          : cloud_type
    use radiation_overlap, only        : calc_overlap_matrices, &
         &  calc_region_fractions
    ...
    real(jprb) :: mu0, one_over_mu0, tan_sza
    ...
    real(jprb) :: od_scaling(2:config%nregions)
    ...
    ! To treat encroachment, we need to know what fraction of diffuse
    ! radiation is scattered during its transmission through a layer
    real(jprb), dimension(config%n_g_sw, config%nregions, nlev) :: frac_scat_diffuse
    ...
    ! Temporary array for applying adding method with encroachment to
    ! albedo matrices
    real(jprb), dimension(config%n_g_sw, config%nregions, config%nregions) &
         &  :: albedo_part

    ! Horizontal migration distance (m) of reflected light, and the
    ! mean number of scattering layers during transit for the diffuse
    ! radiation
    real(jprb), dimension(config%n_g_sw, config%nregions) &
         &  :: x_diffuse, x_direct, n_scat_diffuse
    ! Temporary variables when applying overlap rules
    real(jprb), dimension(config%n_g_sw, config%nregions) &
         &  :: x_diffuse_above, x_direct_above, n_scat_diffuse_above
    ! Two-way migration (m)
    real(jprb), dimension(config%n_g_sw) :: x_total
    ! Factor by which diffuse migration distance is reduced to account
    ! for scattering during transmission
    real(jprb), dimension(config%n_g_sw, config%nregions) :: x_diffuse_scaling
    ...
    ! Fraction of the gridbox associated with each region 
    real(jprb), dimension(config%nregions) :: frac
    ...
    ! Maximum encroachment coefficient
    real(jprb) :: max_encr
    ...
    do jcol = istartcol, iendcol
      ...
      do jlev = 1,nlev ! Start at top-of-atmosphere
        ...
        if (is_clear_sky_layer(jlev)) then
          ...
          if (config%do_3d_effects .and. &
            ...
            if (cloud%inv_cloud_effective_size(jcol,jlev) > 0.0_jprb) then
              ...
              frac = calc_region_fractions(nreg,cloud%fraction(jcol,jlev))
              ...
              do jreg = 1, nreg-1
                ...
                ! zero. The pi on the denominator means that this is
                ! actually edge length with respect to a light ray
                ! with a random azimuthal direction.
                edge_length(jreg,jlev) = four_over_pi &
                     &  * frac(jreg)*(1.0_jprb-frac(jreg)) &
              ...
              if (config%clear_to_thick_fraction > 0.0_jprb &
                   &  .and. nreg > 2) then
              ...
              if (edge_length(3,jlev) > 0.0_jprb) then
                if (frac(1) > epsilon(1.0_jprb)) then
                  transfer_rate_direct(1,3) = dz &
                       &  * edge_length(3,jlev) * tan_sza / frac(1)
                  transfer_rate_diffuse(1,3) = dz &
                       &  * edge_length(3,jlev) &
                       &  * tan_diffuse_angle_3d / frac(1)
                end if
                if (frac(3) > epsilon(1.0_jprb)) then
                  transfer_rate_direct(3,1) = dz &
                       &  * edge_length(3,jlev) * tan_sza / frac(3)
                  transfer_rate_diffuse(3,1) = dz &
                       &  * edge_length(3,jlev) &
                       &  * tan_diffuse_angle_3d / frac(3)
                end if
              end if
          ...
          ! Compute optical depth scaling for the cloudy regions
          call optical_depth_scaling(nreg, cloud%fractional_std(jcol,jlev), &
        ...
        if (config%i_3d_sw_encroachment == IEncroachmentComputed) then
          ! If encroachment is to be computed then we need to know the
          ! probability that a diffuse transmission involves forward
          ! scattering, even in clear layers
          do jreg = 1, nregactive
            call calc_frac_scattered_diffuse_sw(ng, od_region(1:ng,jreg), &
                 &  gamma1(1:ng,jreg), gamma2(1:ng,jreg), &
                 &  frac_scat_diffuse(1:ng,jreg,jlev))
          end do
          if (is_clear_sky_layer(jlev)) then
            frac_scat_diffuse(1:ng,2:nreg,jlev) = 0.0_jprb
          end if
        end if
      ...
      do jreg = 1,nreg
        do jg = 1,ng
          ! frac(jreg) should be the fraction of each region in the
          ! lowest model level
      ...
      x_diffuse = 0.0_jprb
      x_direct  = 0.0_jprb
      ! Initially zero scatterings during transmission
      n_scat_diffuse = 0.0_jprb
      ...
      do jlev = nlev,1,-1
        ...
        ! --------------------------------------------------------
        ! Section 4.2: Overlap and encroachment
        ! --------------------------------------------------------

#ifndef PRINT_ENCROACHMENT_DATA
        if (config%i_3d_sw_encroachment == IEncroachmentComputed &
             &  .and. jlev >= i_cloud_top) then
#else
        if (config%i_3d_sw_encroachment == IEncroachmentComputed) then
#endif
          !  "Computed encroachment": we have the horizontal migration
          ...
          call step_migrations(ng, nreg, cloud%fraction(jcol,jlev), &
               ...
               &  trans_dir_diff(:,:,:,jlev), frac_scat_diffuse(:,:,jlev), &
               &  x_diffuse, x_direct, n_scat_diffuse)

#ifdef PRINT_ENCROACHMENT_DATA
          ! Write out for later analysis: these are the encroachment
          ! statistics at the top of layer "jlev"
          if (nreg == 2) then
            write(101,'(i4,i4,6e14.6)') jcol, jlev, &
                 &  x_direct(1,:), x_diffuse(1,:), n_scat_diffuse(1,:)
          else
            write(101,'(i4,i4,9e14.6)') jcol, jlev, &
                 &  x_direct(1,1:3), x_diffuse(1,1:3), n_scat_diffuse(1,1:3)
          end if
#endif
        ...
        if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev-1)) then
        ...
        else if (config%i_3d_sw_encroachment == IEncroachmentMaximum &
             &  .or. is_clear_sky_layer(jlev-1)) then
          ! "Maximum encroachment": use the overlap matrices u_matrix and v_matrix
          ...
          ! Controlled encroachment: first add the contribution from
          ! off-diagonal elements of the albedo matrix in the lower
          ! layer, i.e. radiation that flows between regions... first
          ! diffuse radiation:
          ...
          if (config%i_3d_sw_encroachment == IEncroachmentMinimum &
               &  .or. (.not. config%do_3d_effects)) then
            ! "Minimum encroachment": the operation we perform is
            ...
            ! "Computed encroachment": Adapt Ornstein-Furth formula
            x_diffuse_scaling &
                 &  = sqrt(2.0_jprb * (n_scat_diffuse+exp(-n_scat_diffuse)-1.0_jprb) + 1.0e-16_jprb) &
                 &  / (n_scat_diffuse+1.0e-8_jprb)
            ...
            do jreg2 = 1,nreg
              ...
              ! Use the following array for diffuse and direct
              ! (despite the name)
              transfer_rate_diffuse = 0.0_jprb

              ! do jreg = 1, nreg-1
              !   ! Compute lateral transfer rates from region jreg to
              !   ! jreg+1 as before, but without length scale which is
              !   ! wavelength dependent
              !   if (u_matrix(jreg2,jreg,jlev,jcol) > epsilon(1.0_jprb)) then
              !     transfer_rate_diffuse(jreg,jreg+1) &
              !          &  = edge_length(jreg,jlev) / u_matrix(jreg2,jreg,jlev,jcol)
              !   end if
              !   ! Compute transfer rates from region jreg+1 to jreg
              !   if (u_matrix(jreg2,jreg+1,jlev,jcol) > epsilon(1.0_jprb)) then
              !     transfer_rate_diffuse(jreg+1,jreg) &
              !          &  = edge_length(jreg,jlev) / u_matrix(jreg2,jreg+1,jlev,jcol)
              !   end if
              ! end do

              ! ! Compute transfer rates directly between regions 1 and 3
              ! if (edge_length(3,jlev) > 0.0_jprb) then
              !   if (u_matrix(jreg2,1,jlev,jcol) > epsilon(1.0_jprb)) then
              !     transfer_rate_diffuse(1,3) &
              !          &  = edge_length(3,jlev) / u_matrix(jreg2,1,jlev,jcol)
              !   end if
              !   if (u_matrix(jreg2,3,jlev,jcol) > epsilon(1.0_jprb)) then
              !     transfer_rate_diffuse(3,1) &
              !          &  = edge_length(3,jlev) / u_matrix(jreg2,3,jlev,jcol)
              !   end if
              ! end if
              ...
              do jreg = 1, nreg-1
                ...
                ! jreg+1 as before, but without length scale which is
                ! wavelength dependent
!                if (u_matrix(jreg,jreg2,jlev,jcol) > epsilon(1.0_jprb)) then
                  transfer_rate_diffuse(jreg,jreg+1) &
                       &  = edge_length(jreg,jlev) / max(u_matrix(jreg,jreg2,jlev,jcol),1.0e-3_jprb)
!                end if
                ! Compute transfer rates from region jreg+1 to jreg
!                if (u_matrix(jreg+1,jreg2,jlev,jcol) > epsilon(1.0_jprb)) then
                  transfer_rate_diffuse(jreg+1,jreg) &
                       &  = edge_length(jreg,jlev) / max(u_matrix(jreg+1,jreg2,jlev,jcol),1.0e-3_jprb)
!                end if
              end do

              ! Compute transfer rates directly between regions 1 and 3
              if (edge_length(3,jlev) > 0.0_jprb) then
!                if (u_matrix(1,jreg2,jlev,jcol) > epsilon(1.0_jprb)) then
                  transfer_rate_diffuse(1,3) &
                       &  = edge_length(3,jlev) / max(u_matrix(1,jreg2,jlev,jcol),1.0e-3_jprb)
!                end if
!                if (u_matrix(3,jreg2,jlev,jcol) > epsilon(1.0_jprb)) then
                  transfer_rate_diffuse(3,1) &
                       &  = edge_length(3,jlev) / max(u_matrix(3,jreg2,jlev,jcol),1.0e-3_jprb)
!                end if
              end if

              ! Compute matrix of exchange coefficients
              encroachment = 0.0_jprb
              do jreg = 1,nreg-1
                ! Diffuse transport down and up with one random
                ! scattering event
                x_total = sqrt(2.0_jprb) * x_diffuse(:,jreg2)*x_diffuse_scaling(:,jreg2)
                encroachment(:,jreg+1,jreg) = encroachment(:,jreg+1,jreg) &
                     &  + transfer_rate_diffuse(jreg,jreg+1)*x_total
                encroachment(:,jreg,jreg) = encroachment(:,jreg,jreg) &
                     &  - encroachment(:,jreg+1,jreg)
                encroachment(:,jreg,jreg+1) = encroachment(:,jreg,jreg+1) &
                     &  + transfer_rate_diffuse(jreg+1,jreg)*x_total
                encroachment(:,jreg+1,jreg+1) = encroachment(:,jreg+1,jreg+1) &
                     &  - encroachment(:,jreg,jreg+1)
              end do
              ...
              do jg = 1,ng
                max_encr = -min(encroachment(jg,1,1),encroachment(jg,2,2))
                if (max_encr > config%max_cloud_od) then
                  ! Scale down all inputs for this g point
                  encroachment(jg,:,:) = encroachment(jg,:,:) * (config%max_cloud_od/max_encr)
                end if
              end do

              ! Use matrix exponential to compute rate of exchange
              albedo_part = encroachment
              ...
              do jreg = 1,nreg-1
                ...
                ! scattering event
                x_total = sqrt(x_direct(:,jreg2)*x_direct(:,jreg2) &
                     &        +(x_diffuse(:,jreg2)*x_diffuse_scaling(:,jreg2))**2)
                encroachment(:,jreg+1,jreg) = encroachment(:,jreg+1,jreg) &
                     &  + transfer_rate_diffuse(jreg,jreg+1)*x_total
                encroachment(:,jreg,jreg) = encroachment(:,jreg,jreg) &
                     &  - encroachment(:,jreg+1,jreg)
        ...
        if (config%i_3d_sw_encroachment == IEncroachmentComputed &
          ...
          do jreg = 1,nreg          ! Target layer (jlev-1)
            do jreg2 = 1,nregactive ! Current layer (jlev)
              ...
              n_scat_diffuse_above(:,jreg) = n_scat_diffuse_above(:,jreg) &
                   &  + n_scat_diffuse(:,jreg2) * v_matrix(jreg2,jreg,jlev,jcol)
            end do
          end do
          ...
          n_scat_diffuse = n_scat_diffuse_above
        end if
      ...
      frac = calc_region_fractions(nreg,cloud%fraction(jcol,1))
      ...
      do jreg = 1,nreg
        direct_dn_below(:,jreg) = incoming_sw(:,jcol)*frac(jreg)
      end do
  ...
  subroutine step_migrations(ng, nreg, cloud_frac, &
       ...
       &  trans_dir_diff, &
       &  frac_scat_diffuse, x_diffuse, x_direct, n_scat_diffuse)
    ...
    ! Fraction of diffuse radiation scattered during its transmission
    ! through a layer
    real(jprb), dimension(ng, nreg) :: frac_scat_diffuse

    ! Inputs/outputs
    ! Horizontal migration distance (m) of reflected light, and the
    ! mean number of scattering layers during transit for the diffuse
    ! radiation
    real(jprb), dimension(ng, nreg) :: x_diffuse, x_direct, n_scat_diffuse
    ...
    !    x_layer_diffuse = layer_depth * tan_diffuse_angle_3d
    x_layer_diffuse = layer_depth * sqrt(3.0_jprb) ! equivalent to mu1=0.5
    !x_layer_diffuse = layer_depth * 2.0_jprb
    x_layer_direct  = layer_depth * tan_sza

    do jreg = istartreg,iendreg
      x_diffuse(:,jreg) = (reflectance(:,jreg,jreg) * 0.5*x_layer_diffuse &
           &  + transmittance(:,jreg,jreg) * (x_diffuse(:,jreg)+x_layer_diffuse)) &
           &  / max(1.0e-6_jprb, reflectance(:,jreg,jreg)+transmittance(:,jreg,jreg))
      ! Note security on n_scat_diffuse
      n_scat_diffuse(:,jreg) = max(0.0_jprb, transmittance(:,jreg,jreg) &
           &  * (frac_scat_diffuse(:,jreg) + n_scat_diffuse(:,jreg)) &
           &  / max(1.0e-6_jprb, reflectance(:,jreg,jreg)+transmittance(:,jreg,jreg)))
!!$      x_direct(:,jreg) = (ref_dir(:,jreg,jreg) * 0.5*x_layer_direct &
!!$           &  + trans_dir_dir(:,jreg,jreg) * (x_direct(:,jreg)+x_layer_direct) &
!!$           &  + trans_dir_diff(:,jreg,jreg)* (x_diffuse(:,jreg)+0.5_jprb*(x_layer_direct+x_layer_diffuse))) &
!!$           &  / max(1.0e-6_jprb, ref_dir(:,jreg,jreg)+trans_dir_dir(:,jreg,jreg) &
!!$           &                                         +trans_dir_diff(:,jreg,jreg))
      x_direct(:,jreg) = (ref_dir(:,jreg,jreg) * 0.5*x_layer_direct &
           &  + trans_dir_dir(:,jreg,jreg) * (x_direct(:,jreg)+x_layer_direct)) &
           &  / max(1.0e-6_jprb, ref_dir(:,jreg,jreg)+trans_dir_dir(:,jreg,jreg))
    end do
    if (iendreg < nreg) then
      ...
      n_scat_diffuse(:,iendreg+1:nreg) = 0.0_jprb
    else if (istartreg == 2) then
      x_diffuse(:,1)      = 0.0_jprb
      x_direct(:,1)       = 0.0_jprb
      n_scat_diffuse(:,1) = 0.0_jprb
    end if
