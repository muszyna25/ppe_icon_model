    ...
    do jcol = istartcol, iendcol
      ...
      do jlev = 1,nlev ! Start at top-of-atmosphere
        ...
        if (.not. is_clear_sky_layer(jlev)) then
          ...
          frac = calc_region_fractions(nregions,cloud%fraction(jcol,jlev))
          do jreg = 1,nregions
            Sup(:,jreg,jlev) = frac(jreg) * Sup(:,jreg,jlev)
            Sdn(:,jreg,jlev) = frac(jreg) * Sdn(:,jreg,jlev)
          end do
        end if
      ...
      frac = calc_region_fractions(nregions,cloud%fraction(jcol,nlev))
      ...
      do jlev = 1,nlev
        ...
        if (.not. (is_clear_sky_layer(jlev) &
          ...
          flux_dn_below = singlemat_x_vec(ng,ng,nregions,v_matrix(:,:,jlev+1,jcol), &
               &  flux_dn)
