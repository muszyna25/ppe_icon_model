...
!        call setup_aerosol_optics_mono(config)
    ...
    if (config%use_canopy_full_spectrum_sw) then
      config%n_canopy_bands_sw = config%n_g_sw
    else if (allocated(config%i_albedo_from_band_sw)) then
      config%n_canopy_bands_sw = maxval(config%i_albedo_from_band_sw)
    else
      config%n_canopy_bands_sw = 1
    end if

    ! Define how many bands to use for reporting surface downwelling
    ! fluxes for canopy radiation scheme
    if (config%use_canopy_full_spectrum_lw) then
      config%n_canopy_bands_lw = config%n_g_lw
    else if (allocated(config%i_emiss_from_band_lw)) then
      config%n_canopy_bands_lw = maxval(config%i_emiss_from_band_lw)
    else
      config%n_canopy_bands_lw = 1
    end if
