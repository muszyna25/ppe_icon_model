  ...
  subroutine solver_spartacus_lw(nlev,istartcol,iendcol, &
    ...
    do jcol = istartcol, iendcol
      ...
      do jlev = 1,nlev ! Start at top-of-atmosphere
        ...
        ! Compute fraction of the gridbox occupied by each region
        ...
        if (ng3D > 0) then
          ...
          do jreg = 1,nregActive
            ...
            planck_top(1:ng3D,nreg+jreg) = od_region(1:ng3D,jreg) &
                 &  *(1.0_jprb-ssa_region(1:ng3D,jreg))*frac(jreg) &
          ...
          if (config%do_lw_side_emissivity &
             & .and. frac(1) > 0.0_jprb .and. frac(2) > 0.0_jprb &
             & .and. config%do_3d_effects &
             & .and. cloud%inv_cloud_effective_size(jcol,jlev) > 0.0_jprb) then
            aspect_ratio = 1.0_jprb / (cloud%inv_cloud_effective_size(jcol,jlev) &
                 &                    * frac(1) * dz)
        ...
        if (ng3D < ng) then
          ...
          source_up(ng3D+1:ng,1,jlev) = frac(1)*source_up_clear(ng3D+1:ng,jlev)
          source_dn(ng3D+1:ng,1,jlev) = frac(1)*source_dn_clear(ng3D+1:ng,jlev)
          ...
          do jreg = 2, nregActive
            call calc_reflectance_transmittance_lw(ng-ng3D, &
                 ...
                 &  frac(jreg)*planck_hl(ng3D+1:ng,jlev,jcol), &
                 &  frac(jreg)*planck_hl(ng3D+1:ng,jlev+1,jcol), &
      ...
      do jreg = 1,nreg
        do jg = 1,ng
          ...
          ! lowest model level
          total_source(jg,jreg,nlev+1) = frac(jreg)*emission(jg,jcol)
