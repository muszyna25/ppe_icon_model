...
! Note array orientation
REAL(KIND=JPRB)   ,INTENT(OUT)   :: POD(JPGPT,YRDIMV%NFLEVG,KIDIA:KFDIA) ! Optical depth
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PSSA(JPGPT,YRDIMV%NFLEVG,KIDIA:KFDIA) ! Single scattering albedo
...
!REAL(KIND=JPRB) :: ZTAU_TOTAL(KIDIA:KFDIA,YRDIMV%NFLEVG) ! Total optical depth
!REAL(KIND=JPRB) :: ZSSA      (KIDIA:KFDIA,YRDIMV%NFLEVG) ! Single scattering albedo
...
DO JB = IB1, IB2
  ...
  DO JG=1,IGT
    DO IC=1,ICOUNT
      ...
      PINCSOL(IW(JL),JL) = ZSFLXZEN(JL,JG)
    ENDDO

!    ZTAU_TOTAL = ZTAUR(:,:,JG) + ZTAUG(:,:,JG)
!    ZSSA = ZTAUR(:,:,JG) / ZTAU_TOTAL
    DO JK=1,KLEV
      DO IC=1,ICOUNT
        JL=INDEX(IC)
        POD (IW(JL),JK,JL) = ZTAUR(JL,JK,JG) + ZTAUG(JL,JK,JG) !ZTAU_TOTAL(JL,JK)
        PSSA(IW(JL),JK,JL) = ZTAUR(JL,JK,JG) / POD (IW(JL),JK,JL) ! ZSSA(JL,JK)
      ENDDO
    ENDDO
