...
The namelist prepicon\_nml contains switches controlling the real-data initialization functionality of ICON.
...
%
\begin{enumerate}
\item \textbf{generate the three-dimensional coordinate fields} needed by IFS2ICON if IFS2ICON is requested to do
the horizontal and vertical interpolation from the IFS grid to the ICON grid
\item \textbf{convert the hydrostatic set of variables provided by IFS2ICON} to the nonhydrostatic set of equations
needed by ICONAM,
\item \label{item:c} \textbf{perform the vertical interpolation} to the ICON grid if IFS2ICON is requested to do only
the horizontal interpolation step,
\item  \label{item:d} \textbf{perform data remapping from a regular grid onto a triangular ICON grid and vice versa}.
\end{enumerate}}

\item
If ICONAM (iequations=3) is combined with NWP physics (iforcing=3), setting ltestcase=.false. activates functionality \ref{item:c} while running
the ICON executable.
\end{itemize}

\textbf{Remark:}
When using prep\_icon as a stand-alone tool, namelists must be defined in a file ``NAMELIST\_PREPICON''.
...
% remap_nml
%------------------------------------------------------------------------------
\subsection{remap\_nml}

The namelist remap\_nml is only processed when setting i\_oper\_mode=4 in prepicon\_nml (cf.\ option \ref{item:d}).
...
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
in\_filename&
C & ````& & input data file name (may be identical to input grid file name) &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline
...
I & 1 & &
Type of input grid: 
1: regular grid; 2: ICON triangular grid
&
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline
...
\hline 
out\_grid\_filename&
C & ````& & file containing output grid that is read as input for the remapping process &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
out\_filename&
C & ````& & output data file name &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline
...
\hline 
s\_maxsize&
I & 500\,000& & Maximum size of sequential list for very large stencils &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
l\_have3dbuffer&
L & .FALSE.& & Flag. True if a 3D buffer is used for output &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
in\_file\_gribedition&
I & 2& & GRIB edition number of input data file &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\end{longtab}

Defined and used in: src/prep\_icon/mo\_remap\_io.f90


%------------------------------------------------------------------------------
% input_field_nml
%------------------------------------------------------------------------------
\subsection{input\_field\_nml}

The namelist input\_field\_nml is only processed when setting i\_oper\_mode=4 in prepicon\_nml (cf.\ option \ref{item:d}).

\begin{longtab}

\hline 
inputname &
C & ```` & & Name of input variable &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
outputname &
C & ```` & & Name of output variable &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
code &
I & 0 & & GRIB input code number &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
type\_of\_layer &
C & ```` & & GRIB input layer type &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
lhydrostatic\_correction &
L & F & & Flag. True, if hydrostatic correction shall be applied to this field &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
var\_temp &
C & ``T'' & & field name: temperature &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline

\hline 
code\_temp &
I & 130 & & GRIB1 code: temperature &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
\hline 
var\_geosp &
C & ``FI'' & & field name: surface geopotential &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
\hline 
code\_geosp &
I & 129 & & GRIB1 code: surface geopotential &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
\hline 
var\_qv &
C & ``QV'' & & field name: specific humidity &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
\hline 
code\_qv &
I & 133 & & GRIB1 code: specific humidity &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
\hline 
hpbl1 &
R & 500 & & height above ground of surface inversion top &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
\hline 
hpbl2 &
R & 1000 & & top of layer used to estimate the vertical temperature gradient above the inversion &
i\_oper\_mode=4 (prepicon\_nml) and
lhydrostatic\_correction=.TRUE.
\tabularnewline
...
Defined and used in: src/prep\_icon/mo\_remap\_input.f90
...
\hline
i\_sea\_ice    & I & 0  && 1: Winton, 2:Semtner& \tabularnewline 
