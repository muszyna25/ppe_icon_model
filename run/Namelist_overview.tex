...
nlev\_in&
I & 91 & &
\textbf{DEPRECATED!} Number of model levels of input data (no longer in use)&
\tabularnewline
...
itype\_latbc&
I & 0& &
Type of lateral boundary conditions\\
0: from the previous time step \\
1: from boundary data (only IFS analysis, so far)&
\tabularnewline
...
''\texttt{<path>prepicon\_<gridfile>}''. May contain the \\
keyword <path> which will be substituted by \\
\texttt{latbc\_path}.
&
itype\_latbc = 1
\tabularnewline
...
\begin{changeitem}{initicon\_nml}{ 2013-07-02 }{ 12700 }
  \begin{itemize}
   \item The number of vertical input levels is now read from file.
         The namelist parameter \textbf{nlev\_in} is deprecated and will soon be removed.
  \end{itemize}
\end{changeitem}
