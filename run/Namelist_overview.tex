...
\hline 
lcompute\_vn&
L & .FALSE.& & Flag: .TRUE., if the normal wind shall be computed. 
This requires the specification of input fields ``u'' and ``v''. &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline

\hline 
lcompute\_vt&
L & .FALSE.& & Flag: .TRUE., if the tangential wind at edge midpoints shall be computed.
This requires the specification of input fields ``u'' and ``v''.  &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline
...
\hline 
rbf\_vec\_scale&
R &  0.05 & & Scaling factor for interpolation with radial basis functions.  &
i\_oper\_mode=4 (prepicon\_nml)
\tabularnewline
