...
Cell type\\
3: triangular cells\\
4: quadrilateral cells (not yet available) &
\tabularnewline
...
\hline
 file\_interval &
C&\'' \''& &
Defines the length of a file in terms of an ISO-8601 duration string (see below).
This namelist parameter can be set instead of \texttt{steps\_per\_file}.
&
\tabularnewline
...
\hline
 filename\_format &
C&see description.& &
 Output filename format. Includes keywords \texttt{path}, \texttt{output\_filename}, \texttt{physdom}, \texttt{levtype}, \texttt{levtype\_l}, \texttt{jfile}, \texttt{ddhhmmss}, \texttt{datetime} see below.
...
\hline
 ...
 with name \texttt{ready\_file} and the output date appended.
...
\texttt{datetime}          \>  substituted by ISO 8601 (absolute) date                  \\
\texttt{ddhhmmss}          \>  substituted by (relative) day-hour-minute-second string
\end{tabbing}
...
\begin{changeitem}{output\_nml: filename\_format }{ 2013-12-02 }{ 15068 }
  \begin{itemize}
   \item The string token \texttt{<ddhhmmss>} is now substuted by the \emph{relative} day-hour-minute-second
