...
\subsection{master\_model\_nml (reapeated for each model)}
\begin{longtab}
...
hbot\_qvsubstep&
R& 250000.0& m&
Height above which QV is advected with substepping scheme (do not use except for debugging purposes)&
cell\_type=3 and \\
...
lwrite\_pzlev&
L& .FALSE. & &
activate output on p- and/or z-levels &
iequations=3
\tabularnewline
...
Please note: Opposed to all other namelists, there may be several instances of
output\_nml in the input file, every one defining a list of variables with
...
\hline
 namespace &
C&' '& &
 'DWD' - DWD short names (or 'MPIM', 'CMIP', 'ECMWF') \\
 {\color{red} Currently unused.} \\
...
L(n\_dom) &
.FALSE.&&
Flag. True, if lon-lat interpolation of output variables is desired.&
\tabularnewline
...
L &
.FALSE.&&
Flag. True, if standard grid variable is not written for lon-lat vars.&
\tabularnewline

\hline 
lonlat\_var\_list&
C &
'' 'PS', 'Q7', 'normal\_velocity' ''&&
List of variables for lon-lat interpolation or ``all''.&
\tabularnewline
...
R(n\_dom) &
2.0&deg&
Interpolation to lon-lat grid: resolution.&
\tabularnewline
...
R(n\_dom) &
-180.0&deg&
South western corner of interpolation area (lon/lat).&
\tabularnewline
...
R(n\_dom) &
 180.0&deg&
North eastern corner of interpolation area (lon/lat).
Overrides corresponding ``dimen'' value.
\tabularnewline
...
R(n\_dom) &
0.&deg&
Position of north pole for interpolation grid.&
\tabularnewline
...
I(n\_dom) &
-1&&
Dimensions of interpolation grid.
Computed automatically when a second area corner is provided.
&
\tabularnewline
...
Defined and used in: src/namelists/mo\_lonlat\_intp\_nml.f90
...
I(n\_dom) &
1&&
intitial time step for meteogram output&
\tabularnewline
...
RRTM on reduced grid (RG follows later)
\tabularnewline
...
jabw test case, including moisture.&\tabularnewline
