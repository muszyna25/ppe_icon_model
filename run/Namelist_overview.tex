...
\documentclass[10pt,english,a4paper,twoside,landscape]{article}
...
\begin{longtable}{|>{\raggedright}p{3cm}|>{\raggedright}p{1cm}|>{\raggedright}p{1.5cm}|>{\raggedright}p{1cm}|>{\raggedright}p{6cm}|>{\raggedright}p{4cm}|}
\hline %--- 
Parameter&
Type&
Default&
...
i\_type\_optimize = 1 or 4 \tabularnewline
\hline %--- 
beta\_spring &
R &
0.90&
...
i\_type\_optimize = 4 \tabularnewline
\hline %---
\end{longtable}
...
\begin{longtable}{|>{\raggedright}p{3cm}|>{\raggedright}p{2cm}|>{\raggedright}p{1.5cm}|>{\raggedright}p{1cm}|>{\raggedright}p{6cm}|>{\raggedright}p{4cm}|}
\hline %--- 
Parameter&
Type&
Default&
...
\section{Namelist parameters defining the ICON model (NAMELIST\_ICON)}
...
\begin{longtable}{|>{\raggedright}p{3cm}|>{\raggedright}p{1.5cm}|>{\raggedright}p{1.5cm}|>{\raggedright}p{1cm}|>{\raggedright}p{6cm}|>{\raggedright}p{4cm}|}
\hline %--- 
Parameter&
Type&
Default&
...
.FALSE.&
&
If .TRUE., simulation starts from a model state read from restart file(s).&
\tabularnewline
...
%-------------------------------------------------------------------
% grid_ctl: horizontal grid
%-------------------------------------------------------------------
\subsection{grid\_ctl}
...
1&
&
number of model domains, 1 = global domain only&
\tabularnewline
\hline %--- 
parent\_id&
I(n\_dom-1)&
...
&
ID of parent domain (first entry refers to first nested domain; needs to be
specified only in case of more than one nested domain per grid level)\\ MUST
be the same as in gridref\_ini & n\_dom>1
\tabularnewline
\hline %--- 
lfeedback &
L(n\_dom) &
...
.FALSE.&
&
If set to .true. an additional patch one level below the root patch is allocated and read
so that physics calculations on a coarser grid are possible &
\tabularnewline
\hline %--- 
lredgrid\_phys &
L &
...
Defined and used in: src/shared/mo\_parallel\_ctl.f90
...
31&
&
Number of full levels (atm. or ocean)&
\tabularnewline
...
0&
&
vertical half level of parent domain which coincides with upper boundary of the current domain&
lvert\_nest=.TRUE.
\tabularnewline
\hline %---
lvert\_nest &
...
&
&
Std. time step for ICOHAM R2B04 = 600s&
\tabularnewline
...
&
&
The initial date and time of this run is preset to 2008-09-01 / 00:00:00 UT&
\tabularnewline
ini\_year&
I&
2008&
...
&
&
The end date and time of this run is preset to 2008-09-01 / 01:40:00 UT, which is 10 steps of 600 s after the initial date and time&
\tabularnewline
end\_year&
I&
2008&
...
&
&
If "nsteps" is positive, then nsteps*dtime is used to compute the end date and time of the run.&
\tabularnewline
&
&
&
&
Else if any of the "run\_..." variables is positive, then these are used to compute the end date and time and, using dtime, also "nsteps".&
\tabularnewline
&
&
&
&
Else the initial date and time and the end date and time and the time step are used to compute "nsteps".&
\tabularnewline
nsteps &
I &
0 &
...
0&
&
number of whole days of the run&
\tabularnewline
run\_hour &
I &
0&
&
number of additional hours of the run&
\tabularnewline
run\_minute &
I &
0&
&
number of additional minutes of the run&
\tabularnewline
run\_second &
R &
0.0&
&
number of additional seconds of the run&
\tabularnewline
...
1&
&
Equations and prognostic variables. Use positive indices for the atmosphere and negative indices for the ocean.\\
0: shallow water model\\
1: hydrostatic atmosphere, T\\
2: hydrostatic atm., $\theta$$\cdot$dp\\
...
.TRUE.&
&
Read namelist 'dynamics\_ctl' and compute adiabatic dynamic tendencies&
\tabularnewline
\hline %--- 
ltransport&
L&
.FALSE.&
&
Read namelist 'transport\_ctl' and compute tracer tendencies by transport&
\tabularnewline
\hline %--- 
iforcing&
I&
...
0&
&
topography (not yet implemented)&
\tabularnewline
\hline %--- 
msg\_level&
I &
...
\subsection{dynamics\_ctl (used if run\_nml/ldynamics=.TRUE.)}
...
i\_cell\_type=3\tabularnewline
&
&
&
&
1: Standard Gaussian integral. Hydrostatic atm.~model: for unaveraged normal components,
...
&
&
2: bilinear averaging of divergence&
\tabularnewline
\hline %--- 
divavg\_cntrwgt&
R&
...
lshallow\_water=.FALSE. and itime\_scheme=4\tabularnewline
\hline %--- 
itime\_scheme&
I&
4&
...
&
&
1: pure advection (no dynamics)&
\tabularnewline
&
&
&
&
2: 2 time level semi implicit (not yet implemented)& iequations=1 or 2
\tabularnewline
&
&
&
&
3: 3 time level explicit& iequations=1 or 2
\tabularnewline
&
&
&
&
4: 3 time level with semi implicit correction& iequations=1 or 2
\tabularnewline
&
&
&
&
5: standard 4th-order Runge-Kutta method (4-stage)& iequations=1 or 2
\tabularnewline
&
&
&
&
6: SSPRK(5,4) scheme (5-stage)& iequations=1 or 2
\tabularnewline
&&&&&
\tabularnewline
&
...
&
&
3: same as default, but computation of velocity tendencies in corrector step only& iequations=3 and i\_cell\_type=3
\tabularnewline
&
&
&
&
4: Matsuno scheme& iequations=3 and i\_cell\_type=3
\tabularnewline
&
&
&
&
6: same as default, but usage of velocity tendencies at (nnow+nnew)/2& iequations=3 and i\_cell\_type=3
\tabularnewline
\hline %--- 
ileapfrog\_startup&
I&
...
itime\_scheme= 3 or 4 \tabularnewline
\hline %--- 
asselin\_coeff&
R&
0.1&
...
than si\_cmin&
itime\_scheme=4 and lsi\_3d=.FALSE.\tabularnewline
\hline %--- 
si\_coeff&
R &
1.0 &
...
itime\_scheme=4 \tabularnewline
\hline %--- 
si\_offctr&
R &
0.7 &
&
future implict weight for si correction&
itime\_scheme=4 \tabularnewline
\hline %--- 
si\_rtol &
R &
...
\subsection{nonhydrostatic\_ctl (used if run\_nml/iequations=3)}
...
17500.0&
m&
Height in which Rayleigh damping starts&
\tabularnewline
\hline %--- 
htop\_moist\_proc&
R&
...
250000.0&
m&
Height above which advection of water vapor is turned off (should be larger than 
htop\_moist\_proc; use with care!!!)&
\tabularnewline
\hline %--- 
k2\_updamp\_coeff&
R&
...
i\_cell\_type=6, hdiff\_order=3 (Smagorinski)
\tabularnewline
\hline %--- 
vwind\_offctr&
R&
...
i\_cell\_type=3 .AND. hdiff\_order=5 .AND. lhdiff\_temp = .true.
\tabularnewline
\hline %--- 
thslp\_zdiffu&
R&
...
i\_cell\_type=3 .AND. hdiff\_order=5 .AND. lhdiff\_temp=.true. .AND. l\_zdiffu\_t=.true.
\tabularnewline
\hline %---
thhgtd\_zdiffu&
R&
...
.TRUE.&
&
implicit vertical advection for horizontal velocity and potential temperature (Implicit vertical advection for $\rho$ and $w$ is automatically included in the new 5band matrix solver for divergent modes. For theta, one of Daniels schemes is envisaged for the future.)  &
i\_cell\_type=6
\tabularnewline
\hline %---
\end{longtable}
...
\subsection{sleve\_ctl (used if nonhydrostatic\_ctl/ivctype=2)}
...
for triangles only with iequations=3, for hexagons with hdiff\_order=3 \tabularnewline
\hline %---
\end{longtable}
...
\subsection{transport\_ctl (used if run\_nml/ltransport=.TRUE.)}
...
2&
&
Horiz. transport scheme:&
\tabularnewline
&
&
4&
&
0: no horiz. transport &
\tabularnewline
&
&
&
...
&
&
2: miura (2nd order, lin. reconstr.)&
if i\_cell\_type=3
\tabularnewline
&
&
...
lsq\_high\_ord $\in$ [2,3]
\tabularnewline
&
&
&
&
4: up3 (3rd or 4th order upstream)&
if i\_cell\_type=6
\tabularnewline
\hline %--- 
ivadv\_tracer&
...
&
&
2: muscl\_cfl (2nd order, handles $\mathrm{CFL}>1$)&
\tabularnewline
&
&
&
...
&
&
3: ppm\_cfl ($3^{\mathrm{rd}}$ order, handles $\mathrm{CFL}>1$)&
\tabularnewline
&
&
&
...
.TRUE.&
&
calculate vertical tracer advection&
\tabularnewline
\hline %--- 
lstrang &
L&
...
&
&
- second order Strang splitting (.TRUE.) &
\tabularnewline
&
&
&
&
- first order Godunov splitting (.FALSE.)&
\tabularnewline
\hline %--- 
ctracer\_list&
C&
...
ihadv\_tracer='miura'\tabularnewline
&
&
&
&
...
ihadv\_tracer='miura[3]'\tabularnewline
&
&
&
&
4: positive definite flux limiter&
ihadv\_tracer='miura[3]', 'iup3[4]'\tabularnewline
\hline %--- 
itype\_vlimit&
I(ntracer)&
1&
...
1&
&
order of backward trajectory calculation:&
\tabularnewline
&
&
&
...
&
&
2: second order (iterative; currently 1 iteration hardcoded)&
ihadv\_tracer='miura'\tabularnewline
\hline %--- 
igrad\_c\_miura&
I&
1&
&
Method for gradient reconstruction at cell center for 2nd order miura&
\tabularnewline
&
&
&
...
1.0&
&
parameter to select 3rd order (=1) or 4th order (=0) advection, or something inbetween (0..1)&
ihadv\_tracer=iup3
\tabularnewline
\hline %---
ivcfl\_max&
...
\subsection{nwp\_phy\_ctl}

\begin{longtable}{|>{\raggedright}p{3cm}|>{\raggedright}p{2.cm}|>{\raggedright}p{1.2cm}|>{\raggedright}p{1.2cm}|>{\raggedright}p{5.5cm}|>{\raggedright}p{4cm}|}
\hline %--- 
Parameter&
Type&
Default&
...
&
&
2: prognostic total water variance
&
\tabularnewline
&
&
...
&
&
4: clouds from COSMO turbulence scheme
&
\tabularnewline
&
&
...
3 & 
& 
mode for calculation of vertical turbulent flux
divergence of 1-st order variables  \newline
0,1 :only for COSMO                 \newline
2    :explicit                      \newline
3    :implicit without moist corr.  \newline
4    :implicit in conserv. vars.    \newline &
inwp\_turb = 1
\tabularnewline
...
.FALSE. & 
 & 
use explicit moist corrections of the implicitly calculated turbulent diffusion &
inwp\_turb = 1
\tabularnewline
...
.FALSE. & 
 & 
calculation of SSO-wake turbulence production for TKE, if SSO-tendencies 
for momentum are present \newline
.TRUE. is currently operational with COSMO &
inwp\_turb = 1
\tabularnewline
...
500.0 & 
m & 
asymptotic turbulent length scale divided by v' Karman-constant \newline
>0 (say in [100, 1000]) &
inwp\_turb = 1
\tabularnewline
...
500.0 & 
m & 
effective length scale of surface patterns \newline
>=0 & 
inwp\_turb = 1
\tabularnewline
...
0.0 & 
 & 
factor for stability correction of turbulent length scale \newline
[0, 1] &
inwp\_turb = 1
\tabularnewline
...
0.1 & 
m2/s & 
minimal turbulent diffusion coefficient for heat &
inwp\_turb = 1
\tabularnewline
...
0.1 & 
 & 
minimal diffusion coefficient for momentum &
inwp\_turb = 1
\tabularnewline
...
0.2 & 
 & 
length scale factors for turbulent transport of TKE &
inwp\_turb = 1
\tabularnewline
...
2 & 
 & 
type of water cloud diagnosis for turbulence \newline
1: based on relative hum.                    \newline
2: statistical saturation adj. &
inwp\_turb = 1
\tabularnewline
...
2 & 
 & 
mode of cloud representation in turbulence parameterization \newline
...
2: sub grid scale condensation according to 'itype\_wcld' &
inwp\_turb = 1
\tabularnewline
...
>=0 (the larger the more laminar transport resistance &
inwp\_turb = 1
\tabularnewline
...
0.0 & 
 & 
scaling factor of the laminar boundary layer for momentum \newline
>=0 (near 0.0 due to roughness layer form drag)  &
inwp\_turb = 1
\tabularnewline
...
20.0 & 
 & 
ratio of laminar scaling factors for heat over sea and land \newline
>1 (since uppermost water layer is moving with the wind) &
inwp\_turb = 1
\tabularnewline
...
\begin{longtable}{|>{\raggedright}p{3cm}|>{\raggedright}p{1.4cm}|>{\raggedright}p{1.5cm}|>{\raggedright}p{1cm}|>{\raggedright}p{6cm}|>{\raggedright}p{4cm}|}
\hline %---
Parameter&
Type&
Default&
...
L&
.TRUE.&&
Switch on large scale condensation.&
iforcing = 2
\tabularnewline
\hline %---
lcover &
...
L&
.FALSE.&&
.TRUE. for subgrid scale orographic drag&
iforcing = 2\par
{\color{red}Not implemeted yet}
\tabularnewline
\hline %---
...
L&
.FALSE.&&
.TRUE. for atmospheric gravity wave drag&
iforcing = 2\par
{\color{red}Not implemeted yet}
\tabularnewline
\hline %---
...
L&
.FALSE.&&
.TRUE. for calculation of meltponds&
iforcing = 2\par
{\color{red}Not implemeted yet}
\tabularnewline
\hline %---
...
L&
.FALSE.&&
.TRUE. for hydrologic discharge model&
iforcing = 2\par
{\color{red}Not implemeted yet}
\tabularnewline
\hline %---
...
L&
.FALSE.&&
.TRUE. for middle atmosphere model version&
iforcing = 2\par
{\color{red}Not implemeted yet}
\tabularnewline
\hline %---
...
R &
0.3 &&
Fractional convective mass flux for downdrafts at lfs (valid range [0,1])
&
iforcing = 2 .AND. lconv = .TRUE.
\tabularnewline
\hline %---
...
I &
1 &&
autoconversion scheme:\\1: Beheng (1994)\par2: Khairoutdinov and Kogan (2000)&
iforcing = 2 .AND. lconv = .TRUE.
\tabularnewline
\hline %---
lconvmassfix &
L &
.FALSE. &&
aerosol mass fixer in convection&
iforcing = 2 .AND. lconv = .TRUE.
\tabularnewline
\hline %---
\end{longtable}

Defined and used in: src/atm\_phy\_echam/mo\_echam\_conv\_parameters.f90

\subsection{echam\_vdiff\_ctl}
...
L&
.TRUE.&&
Switch on surface momentum flux.&
lvdiff = .TRUE.
\tabularnewline
\hline %---
lsfc\_heat\_flux &
L&
.TRUE.&&
Switch on surface sensible and latent heat flux.&
lvdiff = .TRUE.
\tabularnewline
\hline %---
\end{longtable}

Defined and used in: src/namelists/mo\_echam\_vdiff\_nml.f90

%----------------------- end of switches for ECHAM turbulent mixing scheme --------------------
...
.TRUE.&
&
write out averaged vorticity at cells &
\tabularnewline
\hline %--- 
lwrite\_divergence &
L&
...
dtime &
&
diagnostic integral output timestep &
\tabularnewline
\hline %--- 
dt\_file&
R&
2592000 &
s&
Time increment of triggering new output file&
\tabularnewline
\hline %---
dt\_restart&
R & 2592000. & s &
...
.TRUE. if iforcing=iecham or inwp\\
.FALSE. else
\tabularnewline
\hline %---
lwrite\_precip &
...
3&
&
polynomial order for high order reconstruction&
\tabularnewline
&
&
&
...
&
&
30: cubic (no $3^{rd}$ order cross deriv.) &
\tabularnewline
&
&
&
...
&
currently only for i\_cell\_type=6
\tabularnewline
&
&
&
&
1: Almut's method for tangential wind, but PV usage as in TRSK
&
\tabularnewline
&
&
&
&
2: method of Thuburn, Ringler, Skamarock and Klemp (TRSK) 
&
\tabularnewline
&
&
&
&
3: Almut's method for tangential wind and PV usage
&
\tabularnewline
\hline %---
l\_corner\_vort&
...
.TRUE.&
&
switch whether the rhombus averaged corner vorticity is averaged to the hexagon (.TRUE.) or the rhombi are directly averaged to the hexagon (.FALSE.)&
i\_cori\_method=3
\tabularnewline
\hline %---
\end{longtable}
...
&
&
3: combination gradient-based / IDW &
\tabularnewline
&
&
&
...
\subsection{Namelist parameters for testcases (NAMELIST\_ICON)}
...
\subsubsection{testcase\_ctl (Scope: ltestcase=.TRUE. and iequations=[0,1,2] in run\_nml)}
...
&
'SW\_GW': gravity wave&
lshallow\_water=.TRUE.\tabularnewline
&
&
&
&
...
lshallow\_water=.FALSE.\\and iforcing=4\tabularnewline
&
&
&
&
'LDF-Moist': local diabatic forcing test with physics initalised with zonal wind field &
lshallow\_water=.FALSE.,\\and iforcing=5\tabularnewline
&
&
&
&
...
ctest\_name= 'Will\_2', 'Will\_3', 'JWs', 'JWw', 'PA', 'DF1234', \tabularnewline
\hline %--- 
gw\_brunt\_vais&
R&
0.01&
...
ctest\_name= 'MRW(2)'\tabularnewline
\hline %--- 
mountctr\_lat\_deg&
R&
30.0&
...
Defined and used in: src/testcases/mo\_hydro\_testcases.f90

\subsubsection{nh\_testcase\_ctl (Scope: ltestcase=.TRUE. and iequations=3 in run\_nml)}

\begin{longtable}{|>{\raggedright}p{4cm}|>{\raggedright}p{1cm}|>{\raggedright}p{1.5cm}|>{\raggedright}p{1cm}|>{\raggedright}p{6cm}|>{\raggedright}p{4cm}|}
\hline %--- 
Parameter&
Type&
Default&
...
20.0&
m/s&
wind speed for mwbr\_const case&
...
deg&
Earth's rotation axis pitch angle&
nh\_test\_name= 'PA' \tabularnewline
\hline %---
lhs\_nh\_vn\_ptb&
L&
.TRUE.&
&
Add random noise to the initial wind field in the Held-Suarez test. &
nh\_test\_name= 'HS\_nh'\tabularnewline
\hline %--- 
lhs\_fric\_heat&
L&
.FALSE.&
...
nh\_test\_name= 'jabw', nh\_test\_name= 'mrw'\tabularnewline
\hline %---
qv\_max&
R&
20.e-3&
...
\section{Externally provided data}

\subsection{ext\_par\_ctl (Scope: itopo=1 in run\_nml)}
...
Defined and used in: src/namelists/mo\_global\_variables.f90

\subsection{Information on vertical level distribution}
