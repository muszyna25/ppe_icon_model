...
H\_B1\_LK           &      --             &   FG         &     FG      &     FG      &     FG      &     FG      &     FG      &     FG      &    FG         \\
  \bottomrule
\end{longtable}
\footnotetext[1]{Note that $w\_snow$ and $\rho\_snow$ are actually not read from the analysis but from the first guess. $w\_snow$ and $\rho\_snow$ do not contain any 
...
\begin{figure}[hbt]
 \centering
 \includegraphics[width=0.25\textwidth]{analysis_drips.png}
...
itself. Thus in general, using the IAU method requires some care in terms of which fields (with which validity times) must be written to the 
database for later use.
\begin{figure}[hbt]
 \centering
 \includegraphics[width=1.0\textwidth]{IAU_scheme_2.png}
 \caption{Time line for ICON assimilation run starting at 00:00CET.}\label{fig_IAU_scheme}
\end{figure}
