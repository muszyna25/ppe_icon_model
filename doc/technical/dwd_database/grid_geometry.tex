...
ICON setup with 90 vertical levels. The table to the right shows the height above ground of selected model half levels (for zero height topography) 
and the corresponding pressure, assuming the US standard atmosphere. Standard heights for all $91$ half levels are given in Appendix 
\ref{appendix_levelheights}.

\textbf{Please note that these values only provide rough estimates of the true level height for points with non-zero topography. Actual heights may 
...
\subfloat{
  ...
  \caption{Vertical levels of the ICON model (planned operational setup).
