...
%  \begin{itemize}
%    \item Bounds of the refined subregion, with illustration
%    \item No. of cells
%    \item No. of vertical levels, top level height
%    \item Description: The union of four nested triangles is congruent with a parent grid cell
%    \item Mentioning of feedback mechanism
%  \end{itemize}
%  
%  \begin{itemize}
...
\begin{itemize}
  \item Two-way nesting: coupling between forecast on the global grid
        and the regional (Europe) domain.
        \begin{itemize}
          \item boundary data for the nest area is updated every time step (120~s)
          \item feedback of atmospheric prognostic variabes (except precipitation) via relaxation
                on a 3~h time scale.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item geographical coordinates:
        \begin{itemize}
          \item[$\circ$] $23.5^\degree~\text{W}$ -- $62.5^\degree~\text{E}$
          \item[$\circ$] $29.5^\degree~\text{N}$ -- $70.5^\degree~\text{N}$
        \end{itemize}
  \item mesh size: approx. $6.5~\km$ (R3B8)
  \item 60 levels, upper boundary at $22.5~\km$
\end{itemize}

In comparison to: \textbf{COSMO-EU}
\begin{itemize}
  \item rotated geogr. coordinates:
        \begin{itemize}
          \item[$\circ$] $\lambda_{\text{N}} = 170^\degree~\text{W}$, 
                         $\phi_{\text{N}}    =  40^\degree~\text{N}$, 
          \item[$\circ$] $18.0^\degree~\text{W} - 23.5^\degree~\text{E}$
          \item[$\circ$] $20.0^\degree~\text{S} - 21.0^\degree~\text{N}$
        \end{itemize}
  \item mesh size: $0.0625^\degree$ ($\approx 7~\km$)
  \item 665 x 657 grid points
  \item 40 levels, upper boundary at $22.5~\km$
\end{itemize}
...
\begin{figure}[h]
  \begin{minipage}[b]{\textwidth}
    ...
    \includegraphics[width=0.85\textwidth]{ICON_EU_nest_1.png}
  \end{minipage}
  \hfill
  \par
  \begin{minipage}[b]{\textwidth}
    \centering
    \includegraphics[width=0.85\textwidth]{ICON_EU_nest_2.png}
    \subcaption{}\label{fig:EU_nest_polar}
  \end{minipage}\hfill
  \caption{\ref{fig:EU_nest}: Horizontal extent of the ICON-EU nest (orange) in a cylindrical equidistant projection. 
           For comparison, the outline of the COSMO-EU nest is shown in red. \ref{fig:EU_nest_polar}: 
           Same as \ref{fig:EU_nest} but in a polar stereographic projection.}
\end{figure}
