...
\footnotetext[3]{Note that the unit which is displayed, when inspecting the GRIB2 message with \emph{grib\_dump} is $\mathrm{kg\,m^{-2}\,s^{-1}}$ 
...
%\begin{description}[font=\sffamily\bfseries, leftmargin=3.0cm,style=sameline]
\begin{description}[leftmargin=3.0cm,style=sameline]
  \item [CLDEPTH]   Modified cloud depth ($0 \leq \mathrm{CLDEPTH} \leq 1$). Used for visualization purpose (i.e.\ gray-scale pictures) 
                    in the media. A cloud reaching a vertical extent of $700\,\mathrm{hPa}$ or more, has $\mathrm{CLDEPTH}=1$.
