...
\begin{description}[leftmargin=3.0cm,style=sameline]
 ...
 \item [SOILTYP] Characterizes the dominant soiltype in a grid cell. The soiltype is assumed to be the same for all 
                 ...
                 The mapping between these integer numbers and soiltype short names is given in Table~\ref{table_soiltyp}, 
                 together with some soil-dependent hydraulic parameters. For the full list of hydraulic and 
                 thermal parameters, the reader is referred to \cite{Doms2011}.
                 
                 \begin{minipage}[b]{\linewidth}
                   \centering
                   \captionof{table}{Mapping between the the soiltype index stored in the field \texttt{SOILTYP} 
                                     and soiltype short names. The hydraulic parameters \emph{porosity} and 
                                     \emph{field capacity}, currently used by ICON, are given in terms 
                                     of volume fractions.}
                   \label{table_soiltyp}
                  \begin{tabularx}{\linewidth}{llll@{}}
                    \toprule
                    \textbf{index} & \textbf{soiltype} & porosity & field capacity \\
                    \midrule
                    \textbf{1} & \textbf{ice}       &     --  & -- \\
                    \textbf{2} & \textbf{rock}      &     --  & -- \\
                    \textbf{3} & \textbf{sand}      &  0.364  & 0.196 \\
                    \textbf{4} & \textbf{sandyloam} &  0.445  & 0.260 \\
                    \textbf{5} & \textbf{loam}      &  0.455  & 0.340 \\
                    \textbf{6} & \textbf{clayloam}  &  0.475  & 0.370 \\
                    \textbf{7} & \textbf{clay}      &  0.507  & 0.463 \\
                    \textbf{8} & \textbf{peat}      &  0.863  & 0.763 \\
                    \textbf{9} & \textbf{sea water} &     --  & -- \\                
                    \bottomrule
                  \end{tabularx}
                 \end{minipage}
                 \newline
