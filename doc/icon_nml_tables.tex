...
\begin{longtab}
%\hline
echam\_cld\_config(jg)\% \newline ccwmin &
R & 1.0e-7 & kg/kg &
cloud water and ice minimum mass mixing ratio for cover>0 &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline
...
echam\_cld\_config(jg)\% \newline cn1lnd &
R & defval & 1e6/m3 &
cloud droplet number concentration over land, p <= 100 hPa &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline

echam\_cld\_config(jg)\% \newline cn2lnd &
R & defval & 1e6/m3 &
cloud droplet number concentration over land, p >= 800 hPa &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline

echam\_cld\_config(jg)\% \newline cn1sea &
R & defval & 1e6/m3 &
cloud droplet number concentration over sea, p <= 100 hPa &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline

echam\_cld\_config(jg)\% \newline cn2sea &
R & defval & 1e6/m3 &
cloud droplet number concentration over sea, p >= 800 hPa &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline
...
echam\_cld\_config(jg)\% \newline cinhoml1 &
R & 0.8 & &
liquid cloud inhomogeneity factor, ktype = 0 = stratiform clouds &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline

echam\_cld\_config(jg)\% \newline cinhoml2 &
R & 0.4 & &
liquid cloud inhomogeneity factor, ktype = 4 = shallow conv. (cf. clwprat) &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline

echam\_cld\_config(jg)\% \newline cinhoml3 &
R & 0.8 & &
liquid cloud inhomogeneity factor, ktype = 1 = deep convection and ktype = 2 = shallow conv. (cf. clwprat) and ktype = 3 = mid-level conv. &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline
...
echam\_cld\_config(jg)\% \newline jbmin &
I & 40 & &
index of highest level for search of top level of inversion layer over sea (ca. 2 km) &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline

echam\_cld\_config(jg)\% \newline jbmax &
I & 45 & &
index of bottom level of inversion layer over sea &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline
...
echam\_cld\_config(jg)\% \newline csatsc &
R & 0.7 & &
minimum effective saturation for cloud cover below an invesion layer over sea &
echam\_phy\_config(jg)\% \newline dt\_cld > 0.000s
\tabularnewline
...
\begin{longtab}
%\hline
echam\_cnv\_config(jg)\% \newline lmfpen &
L& .TRUE.&&
Switch on penetrative convection.&
echam\_phy\_config(jg)\% \newline dt\_cnv > 0.000s
\tabularnewline
...
4: horizontally constant, vertically decaying, time dependent with surface vol. mixing ratio from file &
echam\_phy\_config(jg)\% \newline dt\_rad > 0.000s \tabularnewline
%
echam\_rad\_config(jg)\% \newline irad\_n2o & I & 3 & &
...
0: set to zero (or epsilon) \\
1: from tracer \\
2: 3-dim concentration, climatological annual cycle, monthly means from an annual file bc\_ozone\_picontrol.nc\\
4: 3-dim concentration, constant in time, 1st time slice file bc\_ozone\_picontrol.nc\\
8: 3-dim concentration, time dependent, monthly means from yearly files bc\_ozone\_historical\_<year>.nc&
echam\_phy\_config(jg)\% \newline dt\_rad > 0.000s \tabularnewline
%
echam\_rad\_config(jg)\% \newline irad\_o2 & I & 2 & &
...
echam\_phy\_config(jg)\% \newline dt\_vdf > 0.000s \tabularnewline
%
\end{longtab}
...
\begin{longtab}
%
echam\_vdf\_config(jg)\% \newline lsfc\_mom\_flux & L & .TRUE. & &
switch on/off surface momentum flux &
echam\_phy\_config(jg)\% \newline dt\_vdf > 0.000s \tabularnewline
%
echam\_vdf\_config(jg)\% \newline lsfc\_heat\_flux & L & .TRUE. & &
...
C & ISO8601 formatted string & date of initial file &
...
C & ISO8601 formatted string & n/a &
...
(grid\_nml: l\_limited\_area = .TRUE.). For the lateral boundary nudging, 
please see interpol\_nml and limarea\_nml. 
...
Please note: \\
- nudge\_type = 1 requires l\_limited\_area = .TRUE. \\
- nudging is applied in primary domian only\\
&
...
the vertical profile of the nudging coefficient reads:\\
...
%\hline
max\_nudge\_coeff\_$\ldots$\\
$\ldots$thermdyn&
R &
0.075 &
1  &
Max. nudging coefficient for the thermodynamic variables 
selected by limarea\_nml: nudge\_hydro\_pres.
The range of validity is 
...
\textbf{operation }&
C&None& &
 Use this variable for internal diagnostics applied on all given output
 variables or groups: \texttt{mean} for generating time averaged, \texttt{max}
 or \texttt{min} for maximum and minimum values within the corresponding
 interval, i.e. \texttt{output\_interval}. \\Supported are 2D, 3D and single
 value like global means on model levels of all components. All operations can
 be used on global and nested grids.
&
\tabularnewline
...
& & & & 4: Piecewise parabolic method (PPM): GPU-enabled version, handles $\mathrm{CFL}>1$& \tabularnewline
