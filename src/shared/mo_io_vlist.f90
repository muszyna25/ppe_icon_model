...
MODULE mo_io_vlist
  ...
  INTEGER, PARAMETER :: max_outvars  = 60 ! max. number of output variables
...
!!$  CALL addVar(DebugVar('debug_3d_8',vlistID(k_jg), gridCellID(k_jg),zaxisID_hybrid(k_jg)), k_jg)
    ...
    IF(lwrite_precip) THEN
      ...
      CASE (inwp)
        ...
        &                   'grid-scale accumulated surface total precipitation',&
        &                   'kg/m**2', 61, 2,&
        &                   vlistID(k_jg), gridCellID(k_jg),zaxisID_surface(k_jg)),&
        &           k_jg)
      CASE (iecham,ildf_echam)
...
!      p_prm_nwp_tend => prm_nwp_tend(jg)
    ...
    SELECT CASE(varname)
      ...
      CASE ('swflxsfc_avg')
        ptr2 => dup2(prm_diag(jg)%swflxsfc_avg(:,:)/z_sim_time); delete = .TRUE.
      CASE ('lwflxsfc_avg')
        ptr2 => dup2(prm_diag(jg)%lwflxsfc_avg(:,:)/z_sim_time); delete = .TRUE.       
      CASE ('swflxtoa_avg')
        ptr2 => dup2(prm_diag(jg)%swflxtoa_avg(:,:)/z_sim_time); delete = .TRUE.
      CASE ('lwflxtoa_avg')
        ptr2 => dup2(prm_diag(jg)%lwflxtoa_avg(:,:)/z_sim_time); delete = .TRUE.
