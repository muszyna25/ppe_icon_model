...
!       & TRIM(gridtype),'R',nroot,'B',level,'_DOM',id,'_proc',p_pe_work,'of',p_n_work,'.nc'
  ...
  SUBROUTINE dump_patch_state_netcdf(p, pi, pg)
    ...
    IF(p_nprocs>1 .AND. p%id>n_dom_start) THEN ! Definitions for local parent
      prefix = 'lp.'
    ...
    IF(p_nprocs>1 .AND. p%id>n_dom_start) THEN ! Output for local parent
      prefix = 'lp.'
...
!       write(0,*) TRIM(filename), " is open."
      ...
      IF(p_nprocs>1 .AND. jg>n_dom_start) THEN
        prefix = 'lp.'
