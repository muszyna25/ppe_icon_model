...
!     IF (lcheck_salt_content) CALL check_total_salt_content_zstar(10, &
!      & p_os%p_prog(nold(1))%tracer(:,:,:,2), p_patch, &
!      & p_os%p_prog(nold(1))%stretch_c(:,:), &
!      & p_patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_c(:,:,:), p_ice, p_oce_sfc)
...
!    IF (lcheck_salt_content) CALL check_total_salt_content_zstar(20, &
!      & p_os%p_prog(nold(1))%tracer(:,:,:,2), p_patch, &
!      & p_os%p_prog(nold(1))%stretch_c(:,:), &
!      & p_patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_c(:,:,:), p_ice, p_oce_sfc)
...
!    IF (lcheck_salt_content) CALL check_total_salt_content_zstar(30, &
!      & p_os%p_prog(nold(1))%tracer(:,:,:,2), p_patch, &
!      & p_os%p_prog(nold(1))%stretch_c(:,:), &
!      & p_patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_c(:,:,:), p_ice, p_oce_sfc)
    ...
    ELSE IF (ice_flux_type .EQ. 2) THEN
      CALL apply_surface_fluxes_zstar_2(p_patch_3D, p_os, p_ice, p_oce_sfc, eta_c, stretch_c)
...
!    IF (lcheck_salt_content) CALL check_total_salt_content_zstar(40, &
!      & p_os%p_prog(nold(1))%tracer(:,:,:,2), p_patch, &
!      & p_os%p_prog(nold(1))%stretch_c(:,:), &
!      & p_patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_c(:,:,:), p_ice, p_oce_sfc)
...
!             & - extra_salt_in_col )/p_ice%zUnderIce(jc,jb)
    ...
    CALL dbg_print('UpdSfc: zUnderIceIni',   zUnderIceIni,                   str_module, 3, in_subset=p_patch%cells%owned)
    CALL dbg_print('UpdSfc: zUnderIceArt',   zUnderIceArt,                   str_module, 3, in_subset=p_patch%cells%owned)
    CALL dbg_print('UpdSfc: ice_draft',      p_ice%draftave,                 str_module, 3, in_subset=p_patch%cells%owned)
    CALL dbg_print('UpdSfc: zUnderIce   ',   p_ice%zUnderIce,                str_module, 2, in_subset=p_patch%cells%owned)
    CALL dbg_print('UpdSfc: sss_inter   ',   sss_inter,                      str_module, 3, in_subset=p_patch%cells%owned)
    CALL dbg_print('UpdSfcEND: oce_sfc%SST ',p_oce_sfc%SST,                  str_module, 2, in_subset=p_patch%cells%owned)
    CALL dbg_print('UpdSfcEND: oce_sfc%SSS ',p_oce_sfc%SSS,                  str_module, 2, in_subset=p_patch%cells%owned)
  ...
  !! even though the variable is still being used
  !
  SUBROUTINE apply_surface_fluxes_zstar_2(p_patch_3D, p_os, p_ice, p_oce_sfc, eta_c, stretch_c)
  ...
  END SUBROUTINE apply_surface_fluxes_zstar_2
