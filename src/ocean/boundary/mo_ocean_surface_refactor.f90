    ...
    DO jb = all_cells%start_block, all_cells%end_block
      ...
      DO jc = i_startidx_c, i_endidx_c
        IF (p_patch_3D%p_patch_1D(1)%dolic_c(jc,jb) > 0) THEN
          ...
          !! All levels need to change since all levels thickness changes
          do level = 2, bt_lev
            p_os%p_prog(nold(1))%tracer(jc, level, jb, 2) = &
              & p_os%p_prog(nold(1))%tracer(jc, level, jb, 2)*stretch_c(jc, jb) &
              & /temp_stretch(jc, jb)
          end do
