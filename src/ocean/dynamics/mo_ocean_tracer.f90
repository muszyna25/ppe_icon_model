...
! END DO
  ...
  !!
!<Optimize:inUse>
  SUBROUTINE advect_individual_tracer_ab_post_step(patch_3d, old_ocean_tracer,       &
    & p_os, p_op_coeff,                    &
    & bc_top_tracer, bc_bot_tracer,p_param,&
    & k_h, a_v,                            &
    & new_ocean_tracer, tracer_index)!,        &
...
!        CALL tracer_diffusion_vert_explicit( patch_3d,            &
!          & old_ocean_tracer%concentration, &
!          & bc_top_tracer,                  &
!          & a_v,                            &
!          & div_diff_flx_vert)
...
!  DO jc = start_cell_index, end_cell_index
  !TODO check algorithm: inv_prism_thick_c vs. del_zlev_m | * vs. /
!  DO level = 1, 5!MIN(patch_3d%p_patch_1d(1)%dolic_c(jc,jb),1)  ! this at most should be 1
!IF(30.0_wp<new_ocean_tracer%concentration(jc,level,jb))THEN 
!write(1030,*)'tracer old-new',jc,level,jb,old_ocean_tracer%concentration(jc,level,jb),new_ocean_tracer%concentration(jc,level,jb) 
!write(1020,*)'tracer old-new',old_ocean_tracer%concentration(9,2,68),new_ocean_tracer%concentration(9,2,68) 
!ENDIF             
!   ENDDO
! END DO
!END DO  
...
! write(1020,*)'------------------------------------'                
  ...
  !!
!<Optimize:inUse>
  SUBROUTINE advect_individual_tracer_ab_during_step(patch_3d, old_ocean_tracer,       &
    & p_os, p_op_coeff,                    &
    & bc_top_tracer, bc_bot_tracer,p_param,&
    & k_h, a_v,                            &
    & new_ocean_tracer, tracer_index)!,        &
...
!ICON_OMP_END_PARALLEL_DO
 Do level=1,n_zlev
  write(0,*)'1st: tracer old-new',level,&
  &maxval(old_ocean_tracer%concentration(:,level,:)),minval(old_ocean_tracer%concentration(:,level,:)),&
  &maxval(new_ocean_tracer%concentration(:,level,:)),minval(new_ocean_tracer%concentration(:,level,:))
  End do
 
old_ocean_tracer%concentration=new_ocean_tracer%concentration
...
!IF(30.0_wp<new_ocean_tracer%concentration(jc,level,jb))THEN 
!write(1030,*)'tracer old-new',jc,level,jb,old_ocean_tracer%concentration(jc,level,jb),new_ocean_tracer%concentration(jc,level,jb) 
!!!write(1020,*)'tracer old-new',old_ocean_tracer%concentration(45,2,184),new_ocean_tracer%concentration(45,2,184) 
!ENDIF           
          ENDDO
! DO level = 1, patch_3d%p_patch_1d(1)%dolic_c(jc,jb)               
! IF(30.0_wp<new_ocean_tracer%concentration(jc,level,jb))THEN 
! write(1030,*)'tracer old-new',jc,level,jb,&
! &patch_3d%p_patch_1d(1)%dolic_c(jc,jb),&
! &old_ocean_tracer%concentration(jc,level,jb),new_ocean_tracer%concentration(jc,level,jb) 
! !!write(1020,*)'tracer old-new',old_ocean_tracer%concentration(45,2,184),new_ocean_tracer%concentration(45,2,184) 
! ENDIF
! END DO           
      ...
      END DO
!ICON_OMP_END_PARALLEL_DO
! write(1020,*)'tracer old-new',old_ocean_tracer%concentration(31,1,63),new_ocean_tracer%concentration(31,1,63)
...
!write(1020,*)'tracer old-new',old_ocean_tracer%concentration(29,2,97),new_ocean_tracer%concentration(29,2,97)
write(1020,*)'------------------------------------'                
