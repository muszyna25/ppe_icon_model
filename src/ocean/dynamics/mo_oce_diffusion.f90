...
!CDIR UNROLL=6
      ...
      DO jk = start_level, elev
        DO jc = start_index, end_index
          ...
          IF (patch_3D%lsm_c(jc,jk,jb) >= boundary) THEN
            z_div_grad_u(jc,jk,jb)%x = 0.0_wp
          ELSE
  ...
  SUBROUTINE veloc_diff_biharmonic_div_grad( patch_3D, p_param, p_diag,&
    ...
    INTEGER :: idx_cartesian
    INTEGER,  DIMENSION(:,:,:),   POINTER :: iidx, iblk
    REAL(wp)                      :: z_laplacian_vn_out(nproma,n_zlev,patch_3D%p_patch_2d(1)%nblks_e)
    TYPE(t_cartesian_coordinates) :: z_grad_u          (nproma,n_zlev,patch_3D%p_patch_2d(1)%nblks_e)
    TYPE(t_cartesian_coordinates) :: z_div_grad_u      (nproma,n_zlev,patch_3D%p_patch_2d(1)%alloc_cell_blocks)
    TYPE(t_subset_range), POINTER :: all_cells, all_edges, edges_in_domain
    TYPE(t_patch), POINTER :: patch_2D
    ...
    laplacian_vn_out  (1:nproma,1:n_zlev,1:patch_3D%p_patch_2d(1)%nblks_e) = 0.0_wp
    z_laplacian_vn_out(1:nproma,1:n_zlev,1:patch_3D%p_patch_2d(1)%nblks_e) = 0.0_wp
...
!CDIR UNROLL=6
    ...
    CALL map_cell2edges_3d( patch_3D, z_div_grad_u,laplacian_vn_out,p_op_coeff)!,subset_range=all_cells)
...
!ICON_OMP_PARALLEL_DO PRIVATE(start_index,end_index, je, jk) ICON_OMP_DEFAULT_SCHEDULE
      DO jb = edges_in_domain%start_block, edges_in_domain%end_block
        CALL get_index_range(edges_in_domain, jb, start_index, end_index)
        nabla2_vec_e(:,:,jb) = 0.0_wp
        DO je = start_index, end_index
          DO jk = start_level, patch_3D%p_patch_1d(1)%dolic_e(je,jb)
            ...
            !              write(0,*)  "3",  patch_2D%edges%system_orientation(je,jb)
            !              write(0,*)  "4",  vort(ividx(je,jb,2),jk,ivblk(je,jb,2))
            !              write(0,*)  "5",  vort(ividx(je,jb,1),jk,ivblk(je,jb,1))
            !              write(0,*)  "6",  patch_2D%edges%inv_primal_edge_length(je,jb)
            !              write(0,*)  "7",  z_div_c(icidx(je,jb,2),jk,icblk(je,jb,2))
            !              write(0,*)  "8",  z_div_c(icidx(je,jb,1),jk,icblk(je,jb,1))
            !              write(0,*)  "9",  patch_2D%edges%inv_dual_edge_length(je,jb)
            !IF(v_base%lsm_e(je,jk,jb) < land_boundary)THEN
            
            nabla2_vec_e(je,jk,jb) = patch_3D%wet_e(je,jk,jb)* &    !v_base%wet_e(je,jk,jb)*&
              & k_h(je,jk,jb) * (                                &
              & patch_2D%edges%system_orientation(je,jb) *        &
      ...
      DO jb = edges_in_domain%start_block, edges_in_domain%end_block
        ...
        DO je = start_index, end_index
          DO jk = start_level, patch_3D%p_patch_1d(1)%dolic_e(je,jb)
            ...
            nabla2_vec_e(je,jk,jb) = patch_3D%wet_e(je,jk,jb)*&
              & (   &
              & patch_2D%edges%system_orientation(je,jb) *     &
              & ( vort(ividx(je,jb,2),jk,ivblk(je,jb,2))     &
              & - vort(ividx(je,jb,1),jk,ivblk(je,jb,1)) )   &
...
!ICON_OMP_END_PARALLEL_DO
  ...
  !-------------------------------------------------------------------------
  SUBROUTINE tracer_diffusion_horz(patch_3D, trac_in, p_os, k_t, diff_flx, subset_range)
    ...
    DO jb = edges_in_domain%start_block, edges_in_domain%end_block
      ...
      DO je = i_startidx_e, i_endidx_e
        ...
        IF (patch_3D%lsm_e(je,jk,jb) <= sea_boundary) THEN
          ...
          delta_z = patch_3D%p_patch_1d(1)%prism_thick_e(je,jk,jb) !v_base%del_zlev_m(start_level) + p_os%p_diag%h_e(je,jb)
        ...
        DO je = i_startidx_e, i_endidx_e
          ...
          IF (patch_3D%lsm_e(je,jk,jb) <= sea_boundary) THEN
            ...
            diff_flx(je,jk,jb) = k_t(je,jk,jb)                           &
              & *patch_3D%p_patch_1d(1)%prism_thick_flat_sfc_e(je,jk,jb)&!  p_os%p_diag%prism_thick_flat_sfc_e(je,jk,jb)&
              & *(trac_in(il_c2,jk,ib_c2)-trac_in(il_c1,jk,ib_c1))&
              & /patch_2D%edges%dual_edge_length(je,jb)
          ENDIF
        ENDDO
    ...
    !CALL div_oce( diff_flx, patch_2D, laplacian_trac_out)
    ...
    DO jb = all_cells%start_block, all_cells%end_block
      ...
      DO jc = start_index, end_index
        ...
        IF ( z_dolic >=min_dolic ) THEN
          !1a) 0cean surface
          !diff_flx(jc,start_level,jb) = A_v(start_level)&
          !& *(top_bc_tracer(jc,jb)-trac_c(jc,start_level,jb))/v_base%del_zlev_i(start_level)
          z_diff_flx(jc,start_level,jb) = top_bc_tracer(jc,jb)
          
          !1b) ocean interior
          DO jk = start_level+1, z_dolic
            ...
            z_diff_flx(jc,jk,jb)&
              & = a_v(jc,jk,jb) &
              & * (trac_c(jc,jk-1,jb)-trac_c(jc,jk,jb))/ patch_3D%p_patch_1d(1)%del_zlev_i(jk)!v_base%del_zlev_i(jk)
          END DO
          ...
          DO jk = 1, z_dolic
            ...
            div_diff_flx(jc,jk,jb) = (z_diff_flx(jc,jk,jb) - z_diff_flx(jc,jk+1,jb))&
              & /patch_3D%p_patch_1d(1)%del_zlev_m(jk)!v_base%del_zlev_m(jk)
          END DO
...
!   SUBROUTINE velocity_diffusion_vert_explicit( patch_3D, p_diag, p_aux,p_op_coeff,&
...
!     INTEGER :: start_index, end_index
!     TYPE(t_cartesian_coordinates) :: z_u(nproma,n_zlev+1,patch_3D%p_patch_2d(1)%alloc_cell_blocks)!,  &
...
!     !-----------------------------------------------------------------------
!
!     z_u(1:nproma,1:n_zlev+1,1:patch_3D%p_patch_2d(1)%alloc_cell_blocks)%x(1) = 0.0_wp
!     z_u(1:nproma,1:n_zlev+1,1:patch_3D%p_patch_2d(1)%alloc_cell_blocks)%x(2) = 0.0_wp
!     z_u(1:nproma,1:n_zlev+1,1:patch_3D%p_patch_2d(1)%alloc_cell_blocks)%x(3) = 0.0_wp
!
!     start_level       = 1
!     elev       = n_zlev
!
!     !1 Vertical derivative of cell velocity vector times horizontal velocity
!     ! loop runs now from start_level to z_dolic:
!     !DO jk = start_level, elev
!     DO jb = cells_in_domain%start_block, cells_in_domain%end_block
!       CALL get_index_range(cells_in_domain, jb, start_index, end_index)
!       DO jc = start_index, end_index
...
!             & /patch_3D%p_patch_1d(1)%del_zlev_i(jk+1))/patch_3D%p_patch_1d(1)%del_zlev_m(jk)
!
!           !1b) ocean bottom
!           jk = z_dolic
!           z_u(jc,jk,jb)%x =                                      &
...
!             & - p_aux%bc_bot_veloc_cc(jc,jb)%x)/patch_3D%p_patch_1d(1)%del_zlev_m(jk)
!
!           !1c) ocean interior
!           DO jk = start_level+1, z_dolic-1
!             z_u(jc,jk,jb)%x = &
...
!     !END DO
!
!     CALL sync_patch_array(sync_c, patch_2D, z_u(:,:,:)%x(1))
!     CALL sync_patch_array(sync_c, patch_2D, z_u(:,:,:)%x(2))
!     CALL sync_patch_array(sync_c, patch_2D, z_u(:,:,:)%x(3))
!     ! Step 2: Map result of previous calculations from cell centers to edges (for all vertical layers)
!     CALL map_cell2edges_3d( patch_3D, z_u,laplacian_vn_out,p_op_coeff)
!     CALL sync_patch_array(sync_e, patch_2D, laplacian_vn_out)
!
!     !---------DEBUG DIAGNOSTICS-------------------------------------------
...
!   END SUBROUTINE velocity_diffusion_vert_explicit
  ...
  !    END DO ! jb = cells_in_domain%start_block, cells_in_domain%end_block
  !
  !    CALL dbg_print('VertTracDiffImpl:residual' ,residual ,str_module,5, in_subset=patch_2D%cells%owned)
  !
...
!ICON_OMP_END_PARALLEL_DO 
  ...
  SUBROUTINE velocity_diffusion_vertical_implicit_onBlock( &
    ...
    DO edge_index = start_index, end_index
      ...
      b(1) = dt_inv - c(1)
      DO jk = 2, bottom_level-1
        a(jk) = - a_v(edge_index,jk)   * inv_prism_thickness(jk) * inv_prisms_center_distance(jk)
        c(jk) = - a_v(edge_index,jk+1) * inv_prism_thickness(jk) * inv_prisms_center_distance(jk+1)
        b(jk) = dt_inv - a(jk) - c(jk)
      END DO
      ! bottom
      ...
      DO jk = 1, bottom_level
        ...
        b(jk)  = diagonal_product
        c(jk)  = dt_inv * fact(jk) - a(jk) - b(jk)
        column_velocity(jk) = velocity(edge_index,jk) * dt_inv * fact(jk)
      ENDDO
      c(bottom_level) = 0.0_wp
      ...
      DO jk = 1, bottom_level
        velocity(edge_index,jk) = column_velocity(jk)
      ENDDO
  ...
  !  dt_inv=1.0_wp/dtime
  !
  !  ! diff_column(1:nproma,1:n_zlev,1:patch_2D%nblks_e)= 0.0_wp
  !
  ...
  !  !---------DEBUG DIAGNOSTICS-------------------------------------------
  !  idt_src=5  ! output print level (1-5, fix)
  !  CALL dbg_print('VelDifImplHomIn:field_col' ,field_column, str_module,idt_src, in_subset=patch_2D%edges%owned)
  !  !---------------------------------------------------------------------
  !
  !  DO jb = all_edges%start_block, all_edges%end_block
  ...
  !  !---------DEBUG DIAGNOSTICS-------------------------------------------
  !  idt_src=5  ! output print level (1-5, fix)
  !  CALL dbg_print('VelDifImplHom: field_col'  ,field_column             ,str_module,idt_src, in_subset=patch_2D%edges%owned)
  !  CALL dbg_print('VelDifImplHom: diff_col'   ,diff_column              ,str_module,idt_src, in_subset=patch_2D%edges%owned)
  !  !---------------------------------------------------------------------
  !
  !END SUBROUTINE velocity_diffusion_vertical_implicit
