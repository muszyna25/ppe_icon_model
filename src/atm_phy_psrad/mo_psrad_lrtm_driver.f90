...
MODULE mo_psrad_lrtm_driver
  ...
  USE mo_psrad_lrtm_kgs, ONLY: ngpt, use_fixed_secdiff, pwvcm_factor
...
CONTAINS
  ...
  SUBROUTINE lrtm(kproma, kbdim, klev, play, psfc, tlay, tlev, tsfc, &
    ...
    gases, ratio, scaleminorn2, fac, laytrop, iabs, &
    ...
    REAL(wp), DIMENSION(:,:), INTENT(IN) :: colbrd, &
    ...
    pwvcm(1:kproma) = SUM(wkl(1:kproma,jSFC:jTOA:jINC,ih2o), DIM=2)
    pwvcm(1:kproma) = pwvcm_factor * (pwvcm(1:kproma) / &
      (SUM(coldry(1:kproma,jSFC:jTOA:jINC), DIM=2) + &
      pwvcm(1:kproma))) * psfc(1:kproma)

    ! Compute radiative transfer for each set of samples
    ig = 0
    DO band = 1,nbndlw
      IF (use_fixed_secdiff(band) == 0) THEN
        CALL fill_secdiff(kproma, band, pwvcm, secdiff)
      ENDIF
