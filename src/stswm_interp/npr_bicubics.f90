      SUBROUTINE npr_bicubics(nlon,nlat,glat,wiy,sg,gifile)
!
...
!          Compute the y-interpolant.
           ...
           s = s1*y2*y3*y4*wiy(1,2,ilat)  &
     &                + s2*y1*y3*y4*wiy(2,2,ilat)      &
     &                + s3*y1*y2*y4*wiy(3,2,ilat)      &
     &                + s4*y1*y2*y3*wiy(4,2,ilat)

!           WRITE(12,'(3e30.16e3)')xlon*180._dp/pi, xlat*180._dp/pi, s
           WRITE(12,*)xlon*180._dp/pi, xlat*180._dp/pi, s
