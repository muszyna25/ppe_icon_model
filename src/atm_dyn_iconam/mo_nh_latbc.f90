...
!
  ...
  PUBLIC :: allocate_latbc_data, read_latbc_data, deallocate_latbc_data, p_latbc_data, &
...
!$OMP END PARALLEL
  ...
  SUBROUTINE read_latbc_data(p_patch, p_nh_state, p_int, ext_data, latbc_datetime, tlev)
    ...
    INTEGER                             :: mpi_comm, ist, dimid, no_cells, &
                                            latbc_fileid, no_levels, varid, jgc, jn
    LOGICAL                             :: l_exist
    ...
    !
    ! perform vertical interpolation of analysis data interpolated horizonally
    ! by IFS2ICON to the ICON grid
    !
  ...
  SUBROUTINE deallocate_latbc_data(patch)
    ...
    INTEGER             :: dimid, no_cells, no_levels, varid
