  ...
  CONTAINS
    ...
    SUBROUTINE h2d_int_state( p_int )
...
#ifdef _CRAYFTN
      INTEGER,  POINTER, DIMENSION(:,:,:)  :: rbf_vec_idx_c, rbf_vec_blk_c, rbf_vec_idx_e, rbf_vec_blk_e, &
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:)  :: c_bln_avg, c_lin_e, cells_aw_verts, e_bln_c_s, e_flx_avg, &
                                              geofac_div, geofac_grdiv, geofac_n2s, geofac_rot, rbf_vec_coeff_e
      REAL(wp), POINTER, DIMENSION(:,:,:,:):: geofac_grg, pos_on_tplane_e, rbf_vec_coeff_c, rbf_vec_coeff_v, &
                                              rbf_c2grad_coeff
#endif
      INTEGER  :: j
...
!$ACC               geofac_div, geofac_grdiv, geofac_grg, geofac_n2s, geofac_rot,  &
!$ACC               nudgecoeff_e, pos_on_tplane_e,                    &
...
!$ACC               p_int(j)%geofac_rot, p_int(j)%nudgecoeff_e, p_int(j)%pos_on_tplane_e,        &
...
!$ACC       IF ( i_am_accel_node  )        
    ...
    SUBROUTINE h2d_patch( p_patch )
      ...
      INTEGER :: j
#ifdef _CRAYFTN
      INTEGER, POINTER, DIMENSION(:,:)   :: refin_ctrl
...
!
      ...
      DO j=1,SIZE(p_patch)
        ...
        refin_ctrl                     =>   p_patch(j)%edges%refin_ctrl
...
!$ACC ENTER DATA &
!$ACC      COPYIN( area, edge_idx, edge_blk, neighbor_idx, neighbor_blk,         &
...
!$ACC              inv_primal_edge_length, inv_dual_edge_length,                 &
!$ACC              tangent_orientation, refin_ctrl, owner_mask,                  &
!$ACC              verts_cell_idx, verts_cell_blk, verts_edge_idx, verts_edge_blk ), &
!$ACC      IF ( i_am_accel_node  )
...
!$ACC              p_patch(j)%edges%cell_blk, p_patch(j)%edges%f_e, p_patch(j)%edges%quad_idx, &
!$ACC              p_patch(j)%edges%quad_blk, p_patch(j)%edges%vertex_idx, p_patch(j)%edges%vertex_blk, &
!$ACC              p_patch(j)%edges%primal_normal_cell, p_patch(j)%edges%dual_normal_cell, &
!$ACC              p_patch(j)%edges%primal_normal_vert, p_patch(j)%edges%dual_normal_vert, &
!$ACC              p_patch(j)%edges%inv_vert_vert_length,   &
!$ACC              p_patch(j)%edges%inv_primal_edge_length, p_patch(j)%edges%inv_dual_edge_length, &
...
!$ACC              p_patch(j)%verts%edge_idx, p_patch(j)%verts%edge_blk ), &
!$ACC      IF ( i_am_accel_node  )
...
!$ACC ENTER DATA COPYIN( vn_traj, mass_flx_me, mass_flx_ic ), IF ( i_am_accel_node  )

#else

!$ACC ENTER DATA COPYIN(prep_adv(j)%vn_traj,prep_adv(j)%mass_flx_me,prep_adv(j)%mass_flx_ic ), &
!$ACC       IF ( i_am_accel_node  )
    ...
    SUBROUTINE h2d_nh_state( p_nh )
      ...
      REAL(wp), POINTER, DIMENSION(:,:)    :: vwind_expl_wgt, vwind_impl_wgt,                  &
      ...
      REAL(vp), POINTER, DIMENSION(:,:,:,:):: coeff_gradp, zdiff_gradp, ddt_vn_adv, ddt_w_adv
...
!$ACC            d_exner_dz_ref_ic, d2dexdz2_fac1_mc,                              &
!$ACC            ddqz_z_half, ddxn_z_full, ddxt_z_full, ddqz_z_full_e,             &
...
!$ACC      COPYIN( ddt_exner_phy, ddt_vn_adv, ddt_vn_phy, ddt_w_adv,               &
!$ACC              div_ic, dtheta_v_ic_ubc, dvn_ie_ubc, dw_ubc, dwdx, dwdy,        &
!$ACC              exner_dyn_incr, exner_incr, exner_pr, grf_bdy_mflx,             &
!$ACC              hdef_ic, mass_fl_e, mflx_ic_ubc,                                &
!$ACC              rho_ic, rho_incr, theta_v_ic, vt, vn_ie, w_concorr_c ),         &
!$ACC      IF ( i_am_accel_node )
...
!$ACC ENTER DATA COPYIN( exner, rho, theta_v, vn, w ), IF ( i_am_accel_node  )
...
!$ACC              p_nh(j)%metrics%ddqz_z_half, p_nh(j)%metrics%ddxn_z_full,                         &
!$ACC              p_nh(j)%metrics%ddxt_z_full, p_nh(j)%metrics%ddqz_z_full_e,                       &
...
!$ACC              p_nh(j)%diag%div_ic, p_nh(j)%diag%dtheta_v_ic_ubc, p_nh(j)%diag%dvn_ie_ubc,       &
...
!$ACC              p_nh(j)%diag%mass_fl_e, p_nh(j)%diag%mflx_ic_ubc,      &
!$ACC              p_nh(j)%diag%rho_ic, p_nh(j)%diag%rho_incr, p_nh(j)%diag%theta_v_ic,              &
!$ACC              p_nh(j)%diag%vt, p_nh(j)%diag%vn_ie, p_nh(j)%diag%w_concorr_c ),                  &
!$ACC      IF ( i_am_accel_node )
...
!$ACC                    p_nh(j)%prog(istep)%theta_v, p_nh(j)%prog(istep)%vn, p_nh(j)%prog(istep)%w ),&
!$ACC      IF ( i_am_accel_node  )
...
!$ACC EXIT DATA &
!$ACC      DELETE( area, edge_idx, edge_blk, neighbor_idx, neighbor_blk,         &
!$ACC              area_edge, cell_idx, cell_blk, f_e, quad_idx, quad_blk,       &  
!$ACC              vertex_idx, vertex_blk, primal_normal_cell, dual_normal_cell, &
!$ACC              primal_normal_vert, dual_normal_vert, inv_vert_vert_length,   &
!$ACC              inv_primal_edge_length, inv_dual_edge_length,                 &
...
!$ACC              p_patch(j)%cells%area, p_patch(j)%cells%edge_idx, p_patch(j)%cells%edge_blk,         &
!$ACC              p_patch(j)%cells%neighbor_idx, p_patch(j)%cells%neighbor_blk, p_patch(j)%cells,      &
!$ACC              p_patch(j)%edges%area_edge, p_patch(j)%edges%cell_idx, p_patch(j)%edges%cell_blk,    &
!$ACC              p_patch(j)%edges%f_e, p_patch(j)%edges%quad_idx, p_patch(j)%edges%quad_blk,          &
!$ACC              p_patch(j)%edges%vertex_idx, p_patch(j)%edges%vertex_blk, &
!$ACC              p_patch(j)%edges%primal_normal_cell, p_patch(j)%edges%dual_normal_cell,              &
!$ACC              p_patch(j)%edges%primal_normal_vert, p_patch(j)%edges%dual_normal_vert,              &
!$ACC              p_patch(j)%edges%inv_vert_vert_length, p_patch(j)%edges%inv_primal_edge_length,      &
!$ACC              p_patch(j)%edges%inv_dual_edge_length, p_patch(j)%edges%tangent_orientation,         &
!$ACC              p_patch(j)%edges%refin_ctrl, p_patch(j)%edges, &
!$ACC              p_patch(j)%verts%cell_idx, p_patch(j)%verts%cell_blk, &
!$ACC              p_patch(j)%verts%edge_idx, p_patch(j)%verts%edge_blk, p_patch(j)%verts  ), &
!$ACC      IF ( i_am_accel_node  )
    ...
    SUBROUTINE d2h_prep_adv( prep_adv )
      ...
      DO j = 1, SIZE( prep_adv )
        ...
        mass_flx_me => prep_adv(j)%mass_flx_me
        mass_flx_ic => prep_adv(j)%mass_flx_ic
!$ACC EXIT DATA DELETE( vn_traj, mass_flx_me, mass_flx_ic ), IF ( i_am_accel_node  )

#else
!$ACC EXIT DATA DELETE(prep_adv(j)%vn_traj,prep_adv(j)%mass_flx_me,prep_adv(j)%mass_flx_ic ), &
!$ACC      IF ( i_am_accel_node  )
#endif
...
!$ACC      DELETE( ddt_exner_phy, ddt_vn_adv, ddt_vn_phy, ddt_w_adv,               &
!$ACC              div_ic, dtheta_v_ic_ubc, dvn_ie_ubc, dw_ubc, dwdx, dwdy,        &
!$ACC              exner_dyn_incr, exner_incr, exner_pr, grf_bdy_mflx,             &
!$ACC              hdef_ic, mass_fl_e, mflx_ic_ubc,                                &
!$ACC              rho_ic, rho_incr, theta_v_ic, vt, vn_ie, w_concorr_c ),         &
...
!$ACC              p_nh(j)%diag%mass_fl_e, p_nh(j)%diag%mflx_ic_ubc,                                 &
!$ACC              p_nh(j)%diag%rho_ic, p_nh(j)%diag%rho_incr, p_nh(j)%diag%theta_v_ic,              &
!$ACC              p_nh(j)%diag%vt, p_nh(j)%diag%vn_ie, p_nh(j)%diag%w_concorr_c, p_nh(j)%diag ),    &
!$ACC      IF ( i_am_accel_node )

        DO istep = 1, SIZE( p_nh(j)%prog )

!$ACC EXIT DATA DELETE( p_nh(j)%prog(istep)%exner, p_nh(j)%prog(istep)%rho,                          &
!$ACC                   p_nh(j)%prog(istep)%theta_v, p_nh(j)%prog(istep)%vn, p_nh(j)%prog(istep)%w ),&
!$ACC      IF ( i_am_accel_node  )
