...
MODULE mo_solve_nonhydro
  ...
  SUBROUTINE velocity_tendencies (p_prog, p_patch, p_int, p_metrics, p_diag, z_w_concorr_me, z_kin_hor_e, &
    ...
    REAL(wp), DIMENSION(:,:,:), INTENT(INOUT) :: z_w_concorr_me, z_kin_hor_e, z_vt_ie
...
!DIR$ IVDEP
        ...
        z_theta_v_pr_mc(i_startidx:i_endidx,1) = p_nh%prog(nnow)%theta_v(i_startidx:i_endidx,1,jb) - &
...
!DIR$ IVDEP
          DO jc = i_startidx, i_endidx
            ...
            z_theta_v_pr_mc(jc,jk) = p_nh%prog(nnow)%theta_v(jc,jk,jb) - p_nh%metrics%theta_ref_mc(jc,jk,jb)
...
!DIR$ IVDEP
      DO jc = i_startidx, i_endidx
        ...
        & +dtime_r*(p_nh%diag%ddt_exner(jc,1,jb)+p_nh%diag%ddt_exner_phy(jc,1,jb))
      ENDDO
...
!DIR$ IVDEP
        DO jc = i_startidx, i_endidx
          ...
          &   +dtime_r*(p_nh%diag%ddt_exner(jc,jk,jb)+p_nh%diag%ddt_exner_phy(jc,jk,jb))
