!>
!! Bimodal cloud scheme for EDMF DUALM
!!
...
USE mo_vdfthermo    ,ONLY : vdfthermo
USE mo_vdfsat       ,ONLY : vdfsat
...
DO JK=1,KLEV
  ...
  IF (LLCLOUDWAKE) THEN
   ...
   !PQLAV(JL,JK)     = PQLAV(JL,JK) * 2._JPRB
    PQLAV(JL,JK)     = PQLAV(JL,JK) + ZQLCCPDF(1) * 0.0005_JPRB * PAWAKE(JL,JK)
   !PQLAV(JL,JK)     = PQLAV(JL,JK) + ZQLCCPDF(1) * 0.001_JPRB * PAWAKE(JL,JK)
  
   !PLDIFF(JL,JK) = PLDIFF(JL,JK) + ZQLCCPDF(1) * 0.001_JPRB * PAWAKE(JL,JK)
