...
!!-------------------------------------------------------------------------
!!
  SUBROUTINE art_emission_interface(p_prog_list,ext_data,p_patch,dtime,p_nh_state,&
       &                            p_diag_lnd,&
       &                            current_date,tracer)
  ...
  IF (lart) THEN
    ...
    IF (art_config(jg)%lart_pntSrc) THEN
      ! Point sources
      CALL art_emission_pntSrc(jg, p_art_data(jg)%pntSrc, current_date, dtime, art_atmo%rho,  &
