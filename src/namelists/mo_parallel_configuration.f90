...
CONTAINS
  ...
  !-------------------------------------------------------------------------
  SUBROUTINE check_parallel_nml()
  
    CHARACTER(*), PARAMETER :: method_name = "check_parallel_nml"

  !------------------------------------------------------------
  ! 3.0 check the consistency of the parameters
  !------------------------------------------------------------
  IF (nml_nproma<=0) CALL finish(TRIM(method_name),'"nml_nproma" must be positive')

  ! check n_ghost_rows
  IF (nml_n_ghost_rows<1) THEN
    CALL finish(method_name, &
         & 'n_ghost_rows<1 in parallel_ctl namelist is not allowed')
  END IF

  ! check division_method
  SELECT CASE (nml_division_method)
  ...
  IF (nml_p_test_run .AND. nml_p_nprocs == 1) THEN
    CALL message(method_name, &
  ...
  ENDIF
  ! for safety only
  IF(nml_num_io_procs < 0) nml_num_io_procs = 0
#endif

  ! check l_test_openmp
#ifndef _OPENMP
  IF (nml_l_test_openmp) THEN
    CALL message(method_name, &
...
!-----------------------------------------------------------------------
  ...
  END SUBROUTINE check_parallel_nml
  !-------------------------------------------------------------------------
