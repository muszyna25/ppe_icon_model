...
MODULE parallele_umgebung
  ...
  use mo_mpi, only : my_cart_id=>p_pe
...
MODULE wolken_sedi
  ...
  CHARACTER(len=*), PARAMETER :: routine = 'ice_sb:wolken_sedi'

CONTAINS


  SUBROUTINE rain_sedi_icon (qr,qnr,qc,rainrate,rho,rhocorr,adz,dt, &
      &                      its,ite,kts,kte)
    !*******************************************************************************
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    DO k = kts,kte
        DO i = its,ite
          ...
          qr(i,k) = rho(i,k)*qr(i,k)
          qnr(i,k) = rho(i,k)*qnr(i,k)
          n_r = qnr(i,k) + eps        !..Anzahldichte in SI
          q_r = qr(i,k)  + eps        !..Fluessigwassergehalt in SI
          IF (cloud_typ <= 1) THEN
          ...
          ELSEIF (use_mu_Dm_rain_sedi) THEN
            ...
            IF (qc(i,k) >= eps) THEN ! Seifert (2007)            
              ! UB_20080212>
              !              mue = 2.0
...
! UB_20090316<<
    ...
    DO k = kts,kte
        ...
        DO i = its,ite
          v_nv(i) = 0.5 * (v_n_rain(i,k+1)+v_n_rain(i,k))
          v_qv(i) = 0.5 * (v_q_rain(i,k+1)+v_q_rain(i,k))
        ...
        IF (ANY(c_nv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_nv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_nv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_nv(i) = s_nv(i) + qnr(i,kk)/adz(i,kk)
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_nv(i) = s_nv(i) + qnr(i,kk)/adz(i,kk)*MIN(c_nv(i),1.0d0)
              s_nv(i) = -s_nv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        IF (ANY(c_qv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_qv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_qv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_qv(i) = s_qv(i) + qr(i,kk)/adz(i,kk)
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_qv(i) = s_qv(i) + qr(i,kk)/adz(i,kk)*MIN(c_qv(i),1.0d0)
              s_qv(i) = -s_qv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        DO i = its,ite
          n_fluss(i,k) = MAX(s_nv(i),n_fluss(i,k-1)-qnr(i,k)/(adz(i,k)*dt))
          q_fluss(i,k) = MAX(s_qv(i),q_fluss(i,k-1)-qr(i,k) /(adz(i,k)*dt))
        END DO
    ...
    DO k = kts,kte
        DO i = its,ite
          qnr(i,k) = qnr(i,k) + ( n_fluss(i,k) - n_fluss(i,k-1) )*adz(i,k)*dt
          qr(i,k)  = qr(i,k)  + ( q_fluss(i,k) - q_fluss(i,k-1) )*adz(i,k)*dt
          qr(i,k)  = qr(i,k)  / rho(i,k)
          qnr(i,k) = qnr(i,k) / rho(i,k)
        ENDDO
    ENDDO
    rainrate(its:ite) = - q_fluss(its:ite,kte) ! Regenrate


    RETURN

  END SUBROUTINE rain_sedi_icon

  SUBROUTINE graupel_sedi_icon (qg,ng,rainrate,rho,rhocorr,adz,dt, &
                  &             its,ite,kts,kte) ! dims
    !*******************************************************************************
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      ...
      CALL message(TRIM(routine), "CLOUD graupel_sedi_icon:")

      WRITE (message_text,'(A,D10.3)') "  Coeff. lambda:          c_lam = ",c_lam
      CALL message(" ",TRIM(message_text))
        
      WRITE (message_text,'(A,D10.3)') "  Coeff. sedimentation n: alf_n = ",alf_n
      CALL message(" ",TRIM(message_text))

      WRITE (message_text,'(A,D10.3)') "  Coeff. sedimentation q: alf_q = ",alf_q
      CALL message(" ",TRIM(message_text))

      firstcall = 1
    ENDIF

    WHERE (qg < 0.d0) qg = 0.d0
    WHERE (ng < 0.d0) ng = 0.d0
    ...
    DO k = kts,kte
        DO i = its,ite
          qg(i,k) = rho(i,k)*qg(i,k)
          ng(i,k) = rho(i,k)*ng(i,k)
          x_g = qg(i,k) / (ng(i,k)+eps)
          x_g = MIN(MAX(x_g,graupel%x_min),graupel%x_max)
          lam = ( c_lam * x_g )**(graupel%b_vel) * rhocorr(i,k)
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.1d+0)
          v_q = MAX(v_q,0.1d+0)
    ...
    DO k = kts,kte
        ...
        IF (ANY(c_nv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_nv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_nv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_nv(i) = s_nv(i) + ng(i,kk)/adz(i,kk)
                c_nv(i) = (c_nv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_nv(i) = s_nv(i) + ng(i,kk)/adz(i,kk)*MIN(c_nv(i),1.0d0)
              s_nv(i) = -s_nv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        IF (ANY(c_qv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_qv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_qv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_qv(i) = s_qv(i) + qg(i,kk)/adz(i,kk)
                c_qv(i) = (c_qv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_qv(i) = s_qv(i) + qg(i,kk)/adz(i,kk)*MIN(c_qv(i),1.0d0)
              s_qv(i) = -s_qv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        ! Flux-limiter to avoid negative values
        DO i = its,ite
          n_fluss(i,k) = MAX(s_nv(i),n_fluss(i,k-1)-ng(i,k)/(adz(i,k)*dt))
          q_fluss(i,k) = MAX(s_qv(i),q_fluss(i,k-1)-qg(i,k)/(adz(i,k)*dt))
        END DO
    ...
    DO k = kts,kte
        DO i = its,ite
          ng(i,k) = ng(i,k) + ( n_fluss(i,k) - n_fluss(i,k-1) )*adz(i,k)*dt
          qg(i,k) = qg(i,k) + ( q_fluss(i,k) - q_fluss(i,k-1) )*adz(i,k)*dt
          qg(i,k) = qg(i,k)  / rho(i,k)
          ng(i,k) = ng(i,k)  / rho(i,k)
        ENDDO
    ENDDO
    rainrate(its:ite) = - q_fluss(its:ite,kte) ! Regenrate
  ...
  END SUBROUTINE graupel_sedi_icon

  SUBROUTINE hail_sedi_icon (qh,nh,rainrate,rho,rhocorr,adz,dt, &
               &             its,ite,kts,kte) ! dims
    !*******************************************************************************
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      ...
      CALL message(TRIM(routine), "CLOUD hail_sedi_icon:")
    ...
    DO k = kts,kte
        DO i = its,ite
          qh(i,k) = rho(i,k)*qh(i,k)
          nh(i,k) = rho(i,k)*nh(i,k)
          x_g = qh(i,k) / (nh(i,k)+eps)
          x_g = MIN(MAX(x_g,hail%x_min),hail%x_max)
          lam = ( c_lam * x_g )**(hail%b_vel) * rhocorr(i,k)
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.1d+0)
          v_q = MAX(v_q,0.1d+0)
    ...
    DO k = kts,kte
        ...
        IF (ANY(c_nv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_nv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_nv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_nv(i) = s_nv(i) + nh(i,kk)/adz(i,kk)
                c_nv(i) = (c_nv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_nv(i) = s_nv(i) + nh(i,kk)/adz(i,kk)*MIN(c_nv(i),1.0d0)
              s_nv(i) = -s_nv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        IF (ANY(c_qv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_qv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_qv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_qv(i) = s_qv(i) + qh(i,kk)/adz(i,kk)
                c_qv(i) = (c_qv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_qv(i) = s_qv(i) + qh(i,kk)/adz(i,kk)*MIN(c_qv(i),1.0d0)
              s_qv(i) = -s_qv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        ! Flux-limiter to avoid negative values
        DO i = its,ite
          n_fluss(i,k) = MAX(s_nv(i),n_fluss(i,k-1)-nh(i,k)/(adz(i,k)*dt))
          q_fluss(i,k) = MAX(s_qv(i),q_fluss(i,k-1)-qh(i,k)/(adz(i,k)*dt))
        END DO
    ...
    DO k = kts,kte
        DO i = its,ite
          nh(i,k) = nh(i,k) + ( n_fluss(i,k) - n_fluss(i,k-1) )*adz(i,k)*dt
          qh(i,k) = qh(i,k) + ( q_fluss(i,k) - q_fluss(i,k-1) )*adz(i,k)*dt
          qh(i,k) = qh(i,k)  / rho(i,k)
          nh(i,k) = nh(i,k)  / rho(i,k)
        ENDDO
    ENDDO
    rainrate(its:ite) = - q_fluss(its:ite,kte) ! Regenrate
  ...
  END SUBROUTINE hail_sedi_icon

  SUBROUTINE snow_sedi_icon (qs,ns,rainrate,rho,rhocorr,adz,dt, &
               &             its,ite,kts,kte) ! dims
    !*******************************************************************************
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      ...
      CALL message(TRIM(routine), "CLOUD snow_sedi_icon:")
    ...
    DO k = kts,kte
        DO i = its,ite
          qs(i,k) = rho(i,k)*qs(i,k)
          ns(i,k) = rho(i,k)*ns(i,k)
          x_s = qs(i,k) / (ns(i,k)+eps)
          ...
          ENDIF
          lam = ( c_lam * x_s )**(snow%b_vel) * rhocorr(i,k)
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.1d+0)
          v_q = MAX(v_q,0.1d+0)
    ...
    DO k = kts,kte
        ...
        IF (ANY(c_nv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_nv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_nv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_nv(i) = s_nv(i) + ns(i,kk)/adz(i,kk)
                c_nv(i) = (c_nv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_nv(i) = s_nv(i) + ns(i,kk)/adz(i,kk)*MIN(c_nv(i),1.0d0)
              s_nv(i) = -s_nv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        IF (ANY(c_qv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_qv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_qv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_qv(i) = s_qv(i) + qs(i,kk)/adz(i,kk)
                c_qv(i) = (c_qv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_qv(i) = s_qv(i) + qs(i,kk)/adz(i,kk)*MIN(c_qv(i),1.0d0)
              s_qv(i) = -s_qv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        ! Flux-limiter to avoid negative values
        DO i = its,ite
          n_fluss(i,k) = MAX(s_nv(i),n_fluss(i,k-1)-ns(i,k)/(adz(i,k)*dt))
          q_fluss(i,k) = MAX(s_qv(i),q_fluss(i,k-1)-qs(i,k)/(adz(i,k)*dt))
        END DO
    ...
    DO k = kts,kte
        DO i = its,ite
          ns(i,k) = ns(i,k) + ( n_fluss(i,k) - n_fluss(i,k-1) )*adz(i,k)*dt
          qs(i,k) = qs(i,k) + ( q_fluss(i,k) - q_fluss(i,k-1) )*adz(i,k)*dt
          qs(i,k) = qs(i,k)  / rho(i,k)
          ns(i,k) = ns(i,k)  / rho(i,k)
        ENDDO
    ENDDO
    rainrate(its:ite) = - q_fluss(its:ite,kte) ! Regenrate
  ...
  END SUBROUTINE snow_sedi_icon

  SUBROUTINE ice_sedi_icon (qi,ni,rainrate,rho,rhocorr,adz,dt, &
              &             its,ite,kts,kte) ! dims
    !*******************************************************************************
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      ...
      CALL message(TRIM(routine), "CLOUD ice_sedi_icon:")
    ...
    DO k = kts,kte
        DO i = its,ite
          qi(i,k) = rho(i,k)*qi(i,k)
          ni(i,k) = rho(i,k)*ni(i,k)
          x_i = qi(i,k) / (ni(i,k)+eps)
          x_i = MIN(MAX(x_i,ice%x_min),ice%x_max)
          lam = ( c_lam * x_i )**(ice%b_vel) * rhocorr(i,k)
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.0d+0)
          v_q = MAX(v_q,0.0d+0)
    ...
    DO k = kts,kte
        ...
        IF (ANY(c_nv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_nv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_nv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_nv(i) = s_nv(i) + ni(i,kk)/adz(i,kk)
                c_nv(i) = (c_nv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_nv(i) = s_nv(i) + ni(i,kk)/adz(i,kk)*MIN(c_nv(i),1.0d0)
              s_nv(i) = -s_nv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        IF (ANY(c_qv(its:ite) > 1)) THEN
          ...
          DO WHILE (ANY(c_qv(its:ite) > 1) .AND. kk > 2)
            DO i = its,ite
              IF (c_qv(i) > 1) THEN
                cflag(i) = .TRUE.
                s_qv(i) = s_qv(i) + qi(i,kk)/adz(i,kk)
                c_qv(i) = (c_qv(i) - 1) * adz(i,kk-1)/adz(i,kk)
              END IF
            END DO
            kk  = kk - 1
          ...
          DO i = its,ite
            IF (cflag(i)) THEN
              s_qv(i) = s_qv(i) + qi(i,kk)/adz(i,kk)*MIN(c_qv(i),1.0d0)
              s_qv(i) = -s_qv(i) / dt
            END IF
          END DO
        ENDIF
        ...
        ! Flux-limiter to avoid negative values
        DO i = its,ite
          n_fluss(i,k) = MAX(s_nv(i),n_fluss(i,k-1)-ni(i,k)/(adz(i,k)*dt))
          q_fluss(i,k) = MAX(s_qv(i),q_fluss(i,k-1)-qi(i,k)/(adz(i,k)*dt))
        END DO
    ...
    DO k = kts,kte
        DO i = its,ite
          ni(i,k) = ni(i,k) + ( n_fluss(i,k) - n_fluss(i,k-1) )*adz(i,k)*dt
          qi(i,k) = qi(i,k) + ( q_fluss(i,k) - q_fluss(i,k-1) )*adz(i,k)*dt
          qi(i,k) = qi(i,k)  / rho(i,k)
          ni(i,k) = ni(i,k)  / rho(i,k)
        ENDDO
    ENDDO
    rainrate(its:ite) = - q_fluss(its:ite,kte) ! Regenrate
...
CONTAINS
  ...
  IF(isIO() .AND. isdebug)THEN
    ...
    ELSE IF ( use_hetnuc .AND. use_homnuc .AND. .NOT.used0 ) THEN
      WRITE (6, *) "CLOUD ice_nucleation: Homog. and het. nucleation, but wp = 0 in KHL06"
    ELSE IF ( use_hetnuc .AND. use_homnuc .AND. used0 ) THEN
