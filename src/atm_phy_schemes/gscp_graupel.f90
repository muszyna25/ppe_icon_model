...
! index-list based code for the NEC SX Aurora, used because the other variant is not reproducible
...
!! -------------------------
 ...
 REAL    (KIND=wp   ) ::  &    
  ...
  INTEGER ::  &
    ic1, ic2, ic3, ic4, ic5, ic6, ic7a, ic7b, ic8, i1d
...
! timestep for calculations
  ...
  DO iv = iv_start, iv_end
    ...
    zvz0s (iv) = 0.0_wp
    zn0s  (iv) = zn0s0
    reduce_dep(iv) = 1.0_wp  

    zeln7o8qrk   (iv) = 0.0_wp
    zeln7o4qrk   (iv) = 0.0_wp
    zeln27o16qrk (iv) = 0.0_wp
    zeln13o8qrk  (iv) = 0.0_wp
    zeln3o4qsk   (iv) = 0.0_wp
    zeln8qsk     (iv) = 0.0_wp
    zeln6qgk     (iv) = 0.0_wp
    zelnrimexp_g (iv) = 0.0_wp
    zsrmax       (iv) = 0.0_wp
    zssmax       (iv) = 0.0_wp
    zsgmax       (iv) = 0.0_wp
    
    zcsdep       (iv) = 3.367E-2_wp     
    zcidep       (iv) = 1.3E-5_wp
    zcslam       (iv) = 1.e10_wp
...
! *********************************************************************
  ...
  !----------------------------------------------------------------------------
    ...
    DO iv = iv_start, iv_end
        ...
        zrho1o2(iv) = EXP(hlp*x1o2)
        zrhofac_qi(iv) = EXP(hlp*icesedi_exp)
        ...
        ! Reset for subsequent computations
        zpkr  (iv) = 0.0_wp
        zpks  (iv) = 0.0_wp
        zpkg  (iv) = 0.0_wp
        zpki  (iv) = 0.0_wp
...
!$NEC ivdep
    loop_over_qs_prepare: DO i1d = 1, ic1
      ...
      IF (isnow_n0temp == 1) THEN
        ...
        zn0s(iv) = MIN(zn0s(iv),1.e9_wp)
        zn0s(iv) = MAX(zn0s(iv),1.e6_wp)
      ELSEIF (isnow_n0temp == 2) THEN
        ! Calculate n0s using the temperature-dependent moment
        ! relations of Field et al. (2005)
        ztc = tg - t0
        ...
        zn0s(iv) = MIN(zn0s(iv),1.e2_wp*hlp)
        zn0s(iv) = MIN(zn0s(iv),1.e9_wp)
        zn0s(iv) = MAX(zn0s(iv),1.e6_wp)
      ELSE
        ! Old constant n0s
      ...
      IF (zvzs(iv) == 0.0_wp) zvzs(iv) = zlnqsk * ccswxp_ln1o2
...
!$NEC ivdep
    loop_over_qr_sedi: DO i1d = 1, ic2
      ...
      IF (zvzr(iv) == 0._wp) zvzr(iv) = zlnqrk * zvzxp_ln1o2
...
!$NEC ivdep
    loop_over_qg_sedi: DO i1d = 1, ic3
      ...
      IF (zvzg(iv) == 0._wp) zvzg(iv) = zlnqgk * zexpsedg_ln1o2
...
!$NEC ivdep
    loop_over_qi_sedi: DO i1d = 1, ic4
      ...
      IF (zvzi(iv) == 0._wp) zvzi(iv) = zlnqik * zbvi_ln1o2
  ...
  !----------------------------------------------------------------------------
    ...
    DO iv = iv_start, iv_end
        ...
        zpkg(iv)   = MIN( zpkg(iv) , MAX(0._wp,zzag(iv)) )
        ...
        zimg(iv)   = 1.0_wp / (1.0_wp + zvzg(iv) * zdtdh(iv))
        ...
        hlp          = 1._wp/(rho(iv,k) * r_v * t(iv,k))
        zqvsi(iv)    = sat_pres_ice(t(iv,k)) * hlp
        zqvsw(iv)    = sat_pres_water(t(iv,k)) * hlp
    ...
    DO iv = iv_start, iv_end
        ...
        IF (llqi .OR. llqs .OR. llqg ) THEN
          ...
          ENDIF
        ENDIF
        IF (llqr(iv) .AND. qvg+qcg <= zqvsw(iv)) THEN
          ic8 = ic8 + 1
          ivdx8(ic8) = iv
        ENDIF
...
!$NEC ivdep
    loop_over_qr: DO i1d =1, ic1
      ...
      IF (qig+qcg > zqmin) zeln7o8qrk(iv) = EXP(x7o8*zlnqrk)
      IF (tg < ztrfrz) THEN
        zeln7o4qrk(iv)   = EXP(x7o4  *zlnqrk)
        zeln27o16qrk(iv) = EXP(x27o16*zlnqrk)
      ENDIF
      IF (llqi) zeln13o8qrk(iv) = EXP(x13o8*zlnqrk)
    ENDDO loop_over_qr
...
!$NEC ivdep
    loop_over_qs_coeffs: DO i1d =1, ic2
      ...
      IF (qig+qcg > zqmin) zeln3o4qsk(iv) = EXP (x3o4*zlnqsk)
      zeln8qsk(iv) = EXP (0.8_wp *zlnqsk)
...
!$NEC ivdep
    loop_over_qg_coeffs: DO i1d =1, ic3
      ...
      IF (qig+qcg > zqmin) zelnrimexp_g(iv) = EXP(zrimexp_g*zlnqgk)
      zeln6qgk(iv) = EXP (0.6_wp*zlnqgk)
...
!$NEC ivdep
    loop_over_qi_qs: DO i1d =1, ic4
      ...
      IF (llqs) THEN
        zcslam(iv) = EXP(ccslxp * LOG(ccslam * zn0s(iv) / MAX(zqmin,zqsk(iv)) ))
        zcsdep(iv) = 4.0_wp * zn0s(iv) * hlp
      ENDIF
      zcslam(iv) = MIN(zcslam(iv),1.e15_wp)
...
!$NEC ivdep
    loop_over_qc: DO i1d =1, ic6
      ...
      IF( tg > zthn ) THEN
        IF (iautocon == 0) THEN
          ! Kessler (1969) autoconversion rate
          scau(iv) = zccau * MAX( qcg - qc0, 0.0_wp )
          scac(iv) = zcac  * qcg * zeln7o8qrk(iv)
        ELSEIF (iautocon == 1) THEN
          ! Seifert and Beheng (2001) autoconversion rate
          ! with cloud droplet number concentration qnc
          IF (qcg > 1.e-6) THEN
            ztau  = MAX(1.E-30_wp,MIN(1.0_wp-qcg/(qcg+qrg),0.9_wp))
            hlp  = EXP(zkphi2*LOG(ztau))
            zphi = zkphi1 * hlp * (1.0_wp - hlp)**3
            scau(iv) = zconst * qcg*qcg*qcg*qcg/(qnc(iv)*qnc(iv)) &
                      * (1.0_wp + zphi/(1.0_wp - ztau)**2)
            zphi      = (ztau/(ztau+zkphi3))**4
      ...
      IF( k>k_start .AND. k<ke .AND. lred_depgrow ) THEN
        ...
        qcgk_1 = qi(iv,km1) + qs(iv,km1) + qg(iv,km1)

        !! distance from cloud top
        IF( qv(iv,km1) + qc(iv,km1) < zqvsw_up(iv) .AND. qcgk_1 < zqmin ) THEN      ! upper cloud layer
          dist_cldtop(iv) = 0.0_wp    ! reset distance to upper cloud layer
        ELSE
          dist_cldtop(iv) = dist_cldtop(iv) + dz(iv,k)
        END IF
...
! ic7
!$NEC ivdep
    loop1_over_qs_qi_qg: DO i1d =1, ic7a
      iv = ivdx7a(i1d)
   ...
   !   IF (tg<=t0) THEN           ! cold case 
        ...
        IF (llqi) THEN
          ...
          siau(iv)   = zeff * zciau * MAX( qig - qi0, 0.0_wp )
          zmi        = MIN( rhog*qig/znin, zmimax )
          zmi        = MAX( zmi0, zmi )
          znid       = rhog * qig/zmi
          zlnlogmi   = LOG (zmi)
          sidep(iv)  = zcidep(iv) * znid * EXP(0.33_wp * zlnlogmi) * zqvsidiff
          zsvidep    = 0.0_wp
          zsvisub    = 0.0_wp
          ! for sedimenting quantities the maximum 
          ! allowed depletion is determined by the predictor value. 
          ...
          ELSE
            zsimax  = qig*zdtr
          ENDIF
          IF( sidep(iv) > 0.0_wp ) THEN
            IF (lred_depgrow ) THEN
              sidep(iv) = sidep(iv) * reduce_dep(iv)  !FR new: depositional growth reduction
            END IF
            zsvidep = MIN( sidep(iv), zsvmax )
          ELSEIF ( sidep(iv) < 0.0_wp ) THEN
          ...
          IF (qsg > 1.e-7_wp) srcri(iv) = zcrcri * (qig/zmi) * zeln13o8qrk(iv)
        ELSE
          zsimax    =  0.0_wp
          zsvidep   =  0.0_wp
          zsvisub   =  0.0_wp
        ...
        !FR new: depositional growth reduction
        IF (lred_depgrow .AND. ssdep(iv) > 0.0_wp) THEN
          ssdep(iv) = ssdep(iv)*reduce_dep(iv)
        END IF
        ! GZ: This limitation, which was missing in the original graupel scheme,
        ! is crucial for numerical stability in the tropics!
        IF (ssdep(iv) > 0.0_wp) ssdep(iv) = MIN(ssdep(iv), zsvmax-zsvidep)
        ! Suppress depositional growth of snow if the existing amount is too small for a
        ! a meaningful distiction between cloud ice and snow
        IF (qsg <= 1.e-7_wp) ssdep(iv) = MIN(ssdep(iv), 0.0_wp)

        sgdep(iv) = (0.398561_wp-0.00152398_wp*tg                 &
                     + 2554.99_wp/ppg+ 2.6531E-7_wp*ppg) *        &
...
!$NEC ivdep
    loop2_over_qs_qi_qg: DO i1d =1, ic7b
      iv = ivdx7b(i1d)
...
!      ELSE ! tg > 0 - warm case
        ...
        IF ( tg > (t0-ztcrit*zqvsw0diff) ) THEN
          !calculate melting rate
          zx1         = (tg - t0) + zasmel*zqvsw0diff
          ssmelt(iv) = (79.6863_wp/ppg+0.612654E-3_wp)* zx1 * zeln8qsk(iv)
          ssmelt(iv) = MIN (ssmelt(iv),zssmax(iv))
          sgmelt(iv) = (12.31698_wp/ppg+7.39441e-05_wp)* zx1 * zeln6qgk(iv)
          sgmelt(iv) = MIN (sgmelt(iv), zsgmax(iv))
          !deposition + melting, ice particle temperature: t0
          !calculation without howell-factor!
          ssdep(iv)  = (31282.3_wp/ppg+0.241897_wp)       &
                      * zqvsw0diff * zeln8qsk(iv)
          sgdep(iv)  = (0.153907_wp-ppg*7.86703e-07_wp)  &
          ...
          IF (zqvsw0diff < 0.0_wp) THEN
            ...
            ssmelt(iv) = MAX( ssmelt(iv), 0.0_wp )
            sgmelt(iv) = MAX( sgmelt(iv), 0.0_wp )
          ELSE
            !deposition on snow/graupel is interpreted as increase
            !in rain water ( qv --> qr, sconr)
            !therefore,  sconr=(zssdep+zsgdep)
        ...
        ELSE
          !if t<t_crit
          !no melting, only evaporation of snow/graupel
          zqvsidiff  = qvg-zqvsw(iv)
          ssdep(iv) = (0.28003_wp-ppg*0.146293E-6_wp) &
                       * zqvsidiff * zeln8qsk(iv)
          sgdep(iv) = (0.0418521_wp-ppg*4.7524E-8_wp) &
                       * zqvsidiff *zeln6qgk(iv)
          ssdep(iv) = MAX(-zssmax(iv) ,ssdep(iv) )
          sgdep(iv) = MAX(-zsgmax(iv) ,sgdep(iv) )
        ENDIF !t_crit
...
!$NEC ivdep
    ...
    loop_over_all_iv: DO iv = iv_start, iv_end
      ...
      ! limit snow depletion in order to avoid negative values of qs
      zcorr  = 1.0_wp
      IF (ssdep(iv) <= 0._wp) THEN
        zsssum = ssmelt(iv) + sconsg(iv) - ssdep(iv)
        IF(zsssum > 0._wp) THEN
          zcorr  = zssmax(iv) / MAX( zssmax(iv), zsssum )
        ENDIF
        ssmelt(iv) = zcorr * ssmelt(iv)
        sconsg(iv) = zcorr * sconsg(iv)
        ssdep(iv)  = zcorr * ssdep(iv)
      ELSE
        zsssum = ssmelt(iv) + sconsg(iv)
        IF(zsssum > 0._wp) THEN
          zcorr  = zssmax(iv) / MAX( zssmax(iv), zsssum )
        ENDIF
        ssmelt(iv) = zcorr * ssmelt(iv)
      ...
      IF ( k /= ke) THEN
        ...
        IF (qgg+qg(iv,k+1) <= zqmin ) THEN
          zvzg(iv)= 0.0_wp
        ELSE
          zvzg(iv)=zvz0g * EXP(zexpsedg*LOG((qgg+qg(iv,k+1))*0.5_wp*rhog))* zrho1o2(iv)
        ENDIF
        IF (qig+qi(iv,k+1) <= zqmin ) THEN
          zvzi(iv)= 0.0_wp
        ELSE
        ...
        ! Reset local arrays for calculations in next level
        zcrim (iv) = 0.0_wp
        zcagg (iv) = 0.0_wp
        zbsdep(iv) = 0.0_wp
        zvz0s (iv) = 0.0_wp
