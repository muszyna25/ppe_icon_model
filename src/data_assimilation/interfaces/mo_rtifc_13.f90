...
!#endif
  ...
  use rttov_const,          only: fastem_sp,             &! max. number of fastem surface parameters
                                  ...
                                  ngases_max,            &!
                                  ncldtyp,               &!
                                  nwcl_max,              &! !CSt: added in context of testing cloud(:,:,:) dims
                                  baran_ngauss,          &!
...
FTRACE_END('rtifc_fill_input_rad')
  ...
  subroutine rtifc_fill_input_var (status,ropts,press,temp,humi,t2m,q2m,psurf,hsurf,&
                                   ...
                                   ice_scheme,idg,clwde,icede,watertype,id, &
...
#undef DIM_ERROR
    ...
    call rttov_parallel_direct (                            &
           ...
           coefs          = coefs(instrIdx),                                 & ! <--  coefs array
    ...
    if (present(profs)) then
      if (associated(profs)) then
        call rttov_alloc_prof(status_rt,size(profs),profs,         &
             size(profs(1)%p),rttov_opts_def,rttov_dealloc,coefs=coefs(1) &
#ifdef _RTTOV_ARCH_VECTOR
             ,profdat=profdat &
#endif
             )
...
FTRACE_END('dealloc_rttov_arrays')
  ...
  subroutine p_bcast_rttov_coefs (coefs, source, comm)
    ...
    call p_bcast(coefs% coef_mfasis_cld, source, comm) !CSt add this line?
    call p_bcast(coefs% coef_mfasis_aer, source, comm) !CSt add this line?
    call p_bcast(coefs% coef_htfrtc, source, comm) !CSt add this line?
  end subroutine
...
!    integer :: dimensions(37)
    ...
    ! if (associated(coef%pol_phi)) dimensions(1:1) = shape(coef%pol_phi) !coef%fmv_chan
    ! if (associated(coef%pol_fac_v)) dimensions(2:2) = shape(coef%pol_fac_v)
    ! if (associated(coef%pol_fac_h)) dimensions(3:3) = shape(coef%pol_fac_h)
    ! if (associated(coef%ss_rayleigh_ext)) dimensions(4:4) = shape(coef%ss_rayleigh_ext)
    ! if (associated(coef%tstar_wsum_r)) dimensions(5:5) = shape(coef%tstar_wsum_r) !0:coef%nlayers
    ! if (associated(coef%tstarmod_wsum_r  )) dimensions(6:6) = shape(coef%tstarmod_wsum_r) !1:coef%nlayers
    ! if (associated(coef%tstar_uwsum_r    )) dimensions(7:7) = shape(coef%tstar_uwsum_r) !0:coef%nlayers
    ! if (associated(coef%wstar_wsum_r     )) dimensions(8:8) = shape(coef%wstar_wsum_r)
    ! if (associated(coef%wtstar_wsum_r    )) dimensions(9:9) = shape(coef%wtstar_wsum_r)
    ! if (associated(coef%ostar_wsum_r     )) dimensions(10:10) = shape(coef%ostar_wsum_r)
    ! if (associated(coef%co2star_wsum_r   )) dimensions(11:11) = shape(coef%co2star_wsum_r)
    ! if (associated(coef%n2ostar_wsum_r   )) dimensions(12:12) = shape(coef%n2ostar_wsum_r)
    ! if (associated(coef%n2otstar_wsum_r  )) dimensions(13:13) = shape(coef%n2otstar_wsum_r)
    ! if (associated(coef%costar_wsum_r    )) dimensions(14:14) = shape(coef%costar_wsum_r)
    ! if (associated(coef%cotstar_wsum_r   )) dimensions(15:15) = shape(coef%cotstar_wsum_r)
    ! if (associated(coef%ch4star_wsum_r   )) dimensions(16:16) = shape(coef%ch4star_wsum_r)
    ! if (associated(coef%ch4tstar_wsum_r  )) dimensions(17:17) = shape(coef%ch4tstar_wsum_r)
    ! if (associated(coef%so2star_wsum_r   )) dimensions(18:18) = shape(coef%so2star_wsum_r)
    ! if (associated(coef%so2tstar_wsum_r  )) dimensions(19:19) = shape(coef%so2tstar_wsum_r)
    ! if (associated(coef%tstar         )) dimensions(20:20) = shape(coef%tstar)  ! 1:coef%nlayers
    ! if (associated(coef%tstar_r       )) dimensions(21:21) = shape(coef%tstar_r)
    ! if (associated(coef%to3star       )) dimensions(22:22) = shape(coef%to3star)
    ! if (associated(coef%to3star_r     )) dimensions(23:23) = shape(coef%to3star_r)
    ! if (associated(coef%wstar         )) dimensions(24:24) = shape(coef%wstar)
    ! if (associated(coef%wstar_r       )) dimensions(25:25) = shape(coef%wstar_r)
    ! if (associated(coef%ostar         )) dimensions(26:26) = shape(coef%ostar)
    ! if (associated(coef%ostar_r       )) dimensions(27:27) = shape(coef%ostar_r)
    ! if (associated(coef%co2star       )) dimensions(28:28) = shape(coef%co2star)
    ! if (associated(coef%co2star_r     )) dimensions(29:29) = shape(coef%co2star_r)
    ! if (associated(coef%n2ostar       )) dimensions(30:30) = shape(coef%n2ostar)
    ! if (associated(coef%n2ostar_r     )) dimensions(31:31) = shape(coef%n2ostar_r)
    ! if (associated(coef%costar        )) dimensions(32:32) = shape(coef%costar)
    ! if (associated(coef%costar_r      )) dimensions(33:33) = shape(coef%costar_r)
    ! if (associated(coef%ch4star       )) dimensions(34:34) = shape(coef%ch4star)
    ! if (associated(coef%ch4star_r     )) dimensions(35:35) = shape(coef%ch4star_r)
    ! if (associated(coef%so2star       )) dimensions(36:36) = shape(coef%so2star)
    ! if (associated(coef%so2star_r     )) dimensions(37:37) = shape(coef%so2star_r)
  ...
  subroutine p_bcast_rttov_cnt_coef_scatt(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_coef_scatt@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_optp(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_optp@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_optp_data(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_optp_data@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_optp_baran(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_optp_baran@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_coef_mfasis(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_coef_mfasis@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_mfasis_axis(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_mfasis_axis@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_mfasis_lut(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_mfasis_lut@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
  ...
  subroutine p_bcast_rttov_cnt_coef_htfrtc(buffer,source,comm)
    ...
    call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
                   source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_coef_htfrtc@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
 ...
 subroutine p_bcast_rttov_cnt_brdf(buffer,source,comm)
   ...
   call MPI_Bcast(buffer,size(transfer(buffer,(/' '/))), MPI_BYTE, &
        source, lcom, errorcode)
    if (errorcode /= MPI_SUCCESS) &
         call finish('p_bcast_rttov_cnt_brdf@mo_rtifc_13', 'MPI ERROR in MPI_Bcast')
#endif
