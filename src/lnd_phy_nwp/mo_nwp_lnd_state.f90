...
!-------------------------------------------------------------------------
  TYPE t_lnd_state
    ...
    TYPE(t_var_list), ALLOCATABLE  :: lnd_prog_nwp_list(:)
...
!-------------------------------------------------------------------------
  ...
  SUBROUTINE new_nwp_lnd_prog_list( p_jg,kblks, listname, vname_prefix, &
    ...
    INTEGER :: ientr
    INTEGER :: jzt           !< time level counter
...
!DR    shape2d              = (/nproma, kblks   /)
!DR    shape3d_subs         = (/nproma, nsfc_subs, kblks  /)
!DR    shape4d_subs         = (/nproma, nztlev, nsfc_subs,  kblks /)
!DR    shape5d_snow_subs    = (/nproma, nlev_snow, nztlev, nsfc_subs,  kblks /)
!DR    shape5d_soil_subs    = (/nproma, nlev_soil, nztlev, nsfc_subs,  kblks /)


    ! predefined array shapes
    shape2d              = (/nproma,            kblks                    /)
    shape3d_subs         = (/nproma,            kblks,         nsfc_subs /)
    shape4d_subs         = (/nproma,            kblks, nztlev, nsfc_subs /)
    shape5d_snow_subs    = (/nproma, nlev_snow, kblks, nztlev, nsfc_subs /)
    shape5d_soil_subs    = (/nproma, nlev_soil, kblks, nztlev, nsfc_subs /)
    ...
    CALL add_var( prog_list, vname_prefix//'t_gt', p_prog_lnd%t_gt,               &
         & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,  cf_desc, grib2_desc,           &
         & ldims=shape4d_subs, lcontainer=.TRUE., lrestart=.FALSE., lpost=.FALSE. )  

!!$    ! fill the seperate variables belonging to the container t_gt
!!$    ALLOCATE(p_prog_lnd%t_gt_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
!!$          WRITE(csfc_zt,'(i1)') cnt 
!!$          CALL add_ref( prog_list, vname_prefix//'t_gt',                         &
!!$                 & vname_prefix//'t_gt_'//TRIM(csfc_zt),                         &
!!$                 & p_prog_lnd%t_gt_ptr(jzt,jsfc)%p_2d,                           &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                        &
!!$                 & t_cf_var('t_gt_'//TRIM(csfc_zt), '', ''),                     &
...
!!$      ENDDO
    ...
    ! & p_prog_lnd%t_snow(nproma,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%t_snow_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
!!$          WRITE(csfc_zt,'(i2)') cnt  
!!$          CALL add_ref( prog_list, vname_prefix//'t_snow',                       &
!!$                 & vname_prefix//'t_snow_'//TRIM(csfc_zt),                       &
!!$                 & p_prog_lnd%t_snow_ptr(jzt,jsfc)%p_2d,                         &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                        &
!!$                 & t_cf_var('t_snow_'//csfc_zt, '', ''),                         &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%t_snow_mult(nproma,nlev_snow,nblks_c,nztlev,nsfc_subs)
...
!!$                 & vname_prefix//'t_snow_mult_'//TRIM(csfc_zt),                  &
!!$                 & p_prog_lnd%t_snow_mult_ptr(jzt,jsfc)%p_3d,                    &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('t_snow_mult_'//csfc_zt, '', ''),                    &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
...
!!$      ENDDO
    ...
    ! & p_prog_lnd%t_s(nproma,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%t_s_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'t_s_'//TRIM(csfc_zt),                          &
!!$                 & p_prog_lnd%t_s_ptr(jzt,jsfc)%p_2d,                            &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                        &
!!$                 & t_cf_var('t_s_'//csfc_zt, '', ''),                            &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%w_snow(nproma,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%w_snow_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'w_snow_'//TRIM(csfc_zt),                       &
!!$                 & p_prog_lnd%w_snow_ptr(jzt,jsfc)%p_2d,                         &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                        &
!!$                 & t_cf_var('w_snow_'//csfc_zt, '', ''),                         &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%rho_snow(nproma,nblks_c,nztlev,nsfc_subs)
...
!!$                 & vname_prefix//'rho_snow_'//csfc, p_prog_lnd%rho_snow_ptr(jsfc)%p_2d, &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                               &
...
!!$      END DO
    ...
    ! & p_prog_lnd%rho_snow_mult(nproma,nlev_snow,nblks_c,nztlev,nsfc_subs)
    ...
    CALL add_var( prog_list, vname_prefix//'rho_snow_mult', p_prog_lnd%rho_snow_mult,  &
         & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,  cf_desc, grib2_desc,                 &
         & ldims=shape5d_snow_subs, lcontainer=.TRUE., lrestart=.FALSE., lpost=.FALSE.)


!!$    ! fill the seperate variables belonging to the container rho_snow_mult
!!$    ALLOCATE(p_prog_lnd%rho_snow_mult_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
!!$          WRITE(csfc_zt,'(i2)') cnt 
!!$          CALL add_ref( prog_list, vname_prefix//'rho_snow_mult_',                  &
!!$                 & vname_prefix//'rho_snow_mult_'//TRIM(csfc_zt),                   &
!!$                 & p_prog_lnd%rho_snow_mult_ptr(jzt,jsfc)%p_3d,                     &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                            &
!!$                 & t_cf_var('rho_snow_mult_'//csfc_zt, '', ''),                     &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL),    &
!!$                 & ldims=(/nproma,nlev_snow,kblks/) )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%w_i(nproma,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%w_i_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'w_i_'//TRIM(csfc_zt),                             &
!!$                 & p_prog_lnd%w_i_ptr(jzt,jsfc)%p_2d,                               &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                           &
!!$                 & t_cf_var('w_i_'//csfc_zt, '', ''),                               &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL),    &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%t_so(nproma,0:nlev_soil+1,nblks_c,nztlev,nsfc_subs) not allowed by add_var
    ! & p_prog_lnd%t_so(nproma,nlev_soil+2,nblks_c,nztlev,nsfc_subs) 
...
!!$    ALLOCATE(p_prog_lnd%t_so_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'t_so_'//TRIM(csfc_zt),                            &
!!$                 & p_prog_lnd%t_so_ptr(jzt,jsfc)%p_3d,                              &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                            &
!!$                 & t_cf_var('t_so_'//csfc_zt, '', ''),                              &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL),    &
...
!!$      ENDDO
   ...
   ! & p_prog_lnd%w_so(nproma,nlev_soil+1,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%w_so_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'w_so_'//TRIM(csfc_zt),                           &
!!$                 & p_prog_lnd%w_so_ptr(jzt,jsfc)%p_3d,                             &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                           &
!!$                 & t_cf_var('w_so_'//csfc_zt, '', ''),                             &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL),   &
...
!!$      ENDDO
    ...
    ! & p_prog_lnd%w_so_ice(nproma,nlev_soil+1,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%w_so_ice_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'w_so_ice_'//TRIM(csfc_zt),                        &
!!$                 & p_prog_lnd%w_so_ice_ptr(jzt,jsfc)%p_3d,                          &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                            &
!!$                 & t_cf_var('w_so_ice_'//csfc_zt, '', ''),                          &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL),    &
!!$                 & ldims=(/nproma,nlev_soil+1,kblks/) )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%wliq_snow(nproma,nlev_snow,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%wliq_snow_ptr(nztlev,nsfc_subs))
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          WRITE(csfc_zt,'(i2)') jsfc*jzt 
!!$          CALL add_ref( prog_list, vname_prefix//'wliq_snow',                    &
!!$                 & vname_prefix//'wliq_snow_'//TRIM(csfc_zt),                    &
!!$                 & p_prog_lnd%wliq_snow_ptr(jzt,jsfc)%p_2d,                      &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('wliq_snow_'//csfc_zt, '', ''),                      &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%wtot_snow(nproma,nlev_snow,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%wtot_snow_ptr(nztlev,nsfc_subs))
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          WRITE(csfc_zt,'(i2)') jsfc*jzt 
!!$          CALL add_ref( prog_list, vname_prefix//'wtot_snow',                    &
!!$                 & vname_prefix//'wtot_snow_'//TRIM(csfc_zt),                    &
!!$                 & p_prog_lnd%wtot_snow_ptr(jzt,jsfc)%p_2d,                      &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('wtot_snow_'//csfc_zt, '', ''),                      &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_prog_lnd%dzh_snow(nproma,nlev_snow,nblks_c,nztlev,nsfc_subs)
...
!!$    ALLOCATE(p_prog_lnd%dzh_snow_ptr(nztlev,nsfc_subs))
!!$      cnt = 0
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          cnt = cnt + 1
...
!!$                 & vname_prefix//'dzh_snow_'//TRIM(csfc_zt),                        &
!!$                 & p_prog_lnd%dzh_snow_ptr(jzt,jsfc)%p_3d,                          &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                            &
!!$                 & t_cf_var('dzh_snow_'//csfc_zt, '', ''),                          &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL),    &
!!$                 & ldims=(/nproma,nlev_snow,kblks/) )
!!$        END DO
!!$      ENDDO
...
!!$    ALLOCATE(p_diag_lnd%qv_st_ptr(nztlev,nsfc_subs))
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          WRITE(csfc_zt,'(i2)') jsfc*jzt 
!!$          CALL add_ref( diag_list, vname_prefix//'qv_st',                        &
!!$                 & vname_prefix//'qv_st_'//TRIM(csfc_zt),                        &
!!$                 & p_diag_lnd%qv_st_ptr(jzt,jsfc)%p_2d,                          &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('qv_st_'//csfc_zt, '', ''),                          &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ! & p_diag_lnd%h_snow(nproma,nblks_c,nztlev,nsfc_subs)
...
!!$    ! fill the seperate variables belonging to the container qv_st
!!$    ALLOCATE(p_diag_lnd%h_snow_ptr(nztlev,nsfc_subs))
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          WRITE(csfc_zt,'(i2)') jsfc*jzt 
!!$          CALL add_ref( diag_list, vname_prefix//'qv_st',                        &
!!$                 & vname_prefix//'h_snow_'//TRIM(csfc_zt),                       &
!!$                 & p_diag_lnd%h_snow_ptr(jzt,jsfc)%p_2d,                         &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('h_snow_'//csfc_zt, '', ''),                         &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
!!$        END DO
!!$      ENDDO
    ...
    ALLOCATE(p_diag_lnd%freshsnow_ptr(nsfc_subs))
      DO jsfc = 1,nsfc_subs
        WRITE(csfc,'(i1)') jsfc 
        CALL add_ref( diag_list, 'freshsnow',                                      &
                 & vname_prefix//'freshsnow_'//csfc, p_diag_lnd%freshsnow_ptr(jsfc)%p_2d, &
                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                          &
...
!    ! & p_diag_lnd%wliq_snow(nproma,nblks_c,nlev_snow,nztlev,nsfc_subs)
...
!!$    ! fill the seperate variables belonging to the container wliq_snow
!!$    ALLOCATE(p_diag_lnd%wliq_snow_ptr(nztlev,nsfc_subs))
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          WRITE(csfc_zt,'(i2)') jsfc*jzt 
!!$          CALL add_ref( diag_list, vname_prefix//'wliq_snow',                    &
!!$                 & vname_prefix//'wliq_snow_'//TRIM(csfc_zt),                    &
!!$                 & p_diag_lnd%wliq_snow_ptr(jzt,jsfc)%p_2d,                      &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('wliq_snow_'//csfc_zt, '', ''),                      &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
...
!       ! & p_diag_lnd%wtot_snow(nproma,nblks_c,nlev_snow,nztlev,nsfc_subs)
...
!!$    ! fill the seperate variables belonging to the container wtot_snow
!!$    ALLOCATE(p_diag_lnd%wtot_snow_ptr(nztlev,nsfc_subs))
!!$      DO jsfc = 1,nsfc_subs
!!$        DO jzt = 1, nztlev
!!$          WRITE(csfc_zt,'(i2)') jsfc*jzt 
!!$          CALL add_ref( diag_list, vname_prefix//'wtot_snow',                    &
!!$                 & vname_prefix//'wtot_snow_'//TRIM(csfc_zt),                    &
!!$                 & p_diag_lnd%wtot_snow_ptr(jzt,jsfc)%p_2d,                      &
!!$                 & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                         &
!!$                 & t_cf_var('wtot_snow_'//csfc_zt, '', ''),                      &
!!$                 & t_grib2_var(255, 255, 255, ientr, GRID_REFERENCE, GRID_CELL), &
!!$                 & ldims=shape2d )
...
!!$      ENDDO
    ...
    ALLOCATE(p_diag_lnd%runoff_s_ptr(nsfc_subs))
      DO jsfc = 1,nsfc_subs
        WRITE(csfc,'(i1)') jsfc 
        CALL add_ref( diag_list, 'runoff_s',                                            &
                 & vname_prefix//'runoff_s_'//csfc, p_diag_lnd%runoff_s_ptr(jsfc)%p_2d, &
                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                               &
    ...
    ALLOCATE(p_diag_lnd%runoff_g_ptr(nsfc_subs))
      DO jsfc = 1,nsfc_subs
        WRITE(csfc,'(i1)') jsfc 
        CALL add_ref( diag_list, 'runoff_g',                                            &
                 & vname_prefix//'runoff_g_'//csfc, p_diag_lnd%runoff_g_ptr(jsfc)%p_2d, &
                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                               &
    ...
    CALL add_var( diag_list, 'rstom', p_diag_lnd%rstom,                             &
           & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,  cf_desc, grib2_desc,           &
           & ldims=shape2d, lcontainer=.TRUE., lrestart=.FALSE., lpost=.FALSE. )
    ...
    ALLOCATE(p_diag_lnd%lhfl_bs_ptr(nsfc_subs))
      DO jsfc = 1,nsfc_subs
        WRITE(csfc,'(i1)') jsfc 
        CALL add_ref( diag_list, 'lhfl_bs',                                        &
                 & vname_prefix//'lhfl_bs_'//csfc, p_diag_lnd%lhfl_bs_ptr(jsfc)%p_2d, &
                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                          &
    ...
    ALLOCATE(p_diag_lnd%lhfl_pl_ptr(nsfc_subs))
      DO jsfc = 1,nsfc_subs
        WRITE(csfc,'(i1)') jsfc 
        CALL add_ref( diag_list, 'lhfl_pl',                                        &
                 & vname_prefix//'lhfl_pl_'//csfc, p_diag_lnd%lhfl_pl_ptr(jsfc)%p_2d, &
                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                          &
    ...
    ALLOCATE(p_diag_lnd%subsfrac_ptr(nsfc_subs))
      DO jsfc = 1,nsfc_subs
        WRITE(csfc,'(i1)') jsfc 
        CALL add_ref( diag_list, 'subsfrac',                                       &
                 & vname_prefix//'subsfrac_'//csfc, p_diag_lnd%subsfrac_ptr(jsfc)%p_2d, &
                 & GRID_UNSTRUCTURED_CELL, ZAXIS_SURFACE,                          &
