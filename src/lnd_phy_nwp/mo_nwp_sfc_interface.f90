...
!$NEC ivdep
           ...
           DO ic = 1, i_count
             ...
             IF (ext_data%atm%lc_class_t(jc,jb,isubs) == ext_data%atm%i_lc_snow_ice) THEN
               tmp1 = tcall_sfc_jg * 1.e-9_wp * (600._wp-lnd_prog_now%rho_snow_t(jc,jb,isubs))*        &
                 MAX(0._wp,SQRT(SQRT(lnd_diag%freshsnow_t(jc,jb,isubs)))*prm_diag%gust10(jc,jb)-7.5_wp)**2
