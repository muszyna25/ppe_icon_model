...
CONTAINS
  ...
  SUBROUTINE construct_icoham_dyn_state( ntimelevel, p_patch )
    ...
    !local variables
    CHARACTER(len=MAX_CHAR_LENGTH) :: listname, prefix
    ...
    DO jg = 1,ndomain
      ...
      WRITE(prefix,'(a)')        'ha_prog_out_'
      CALL make_hydro_prog_list( nproma, nlev, nblks_c, nblks_e, nblks_v, ntracer,     &
                               & TRIM(listname), TRIM(prefix), hydro_prog_out_list(jg),&
                               & p_hydro_state(jg)%prog_out )

      WRITE(listname,'(a,i2.2)') 'hydro_diag_out_D',jg
      WRITE(prefix,'(a)')        'ha_diag_out_'
      CALL make_hydro_diag_list( nproma, nlev, nblks_c, nblks_e, nblks_v, ntracer,     &
                               & TRIM(listname), TRIM(prefix), hydro_diag_out_list(jg),&
  ...
  SUBROUTINE make_hydro_prog_list( kproma, klev, kblks_c, kblks_e, kblks_v,  & 
    ...
    CALL add_var( field_list, prefix//'vn', field%vn,  &
    ...
    IF (ltheta_dyn) THEN
      ...
      grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
      CALL add_var( field_list, prefix//'theta', field%theta, &
                  & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,     &
                  & cf_desc, grib2_desc, ldims=shape3d_c )
    ENDIF
    ...
    IF (ktracer > 0) THEN
      ...
      CALL add_var( field_list, prefix//'tracer', field%tracer,                 &
                  & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                       &
      ...
      DO jtrc = 1,ktracer
        ...
        CALL add_ref( field_list, prefix//'tracer',                                         &
                    & prefix//'tracer_'//ctracer_list(jtrc:jtrc), field%tracer_ptr(jtrc)%p, &
                    & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,                                 &
...
!    IF (ktracer > 0) field% tracer = 0.0_wp
  ...
  SUBROUTINE make_hydro_diag_list( kproma, klev, kblks_c, kblks_e, kblks_v,  & 
    ...
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_EDGE)
    CALL add_var( field_list, prefix//'vt', field%vt,           &
                & GRID_UNSTRUCTURED_EDGE, ZAXIS_HYBRID, &
                & cf_desc, grib2_desc, ldims=shape3d_e  )
    ...
    CALL add_var( field_list, prefix//'rel_vort', field%rel_vort, &
    ...
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'delp_c_new', field%delp_c_new, &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,       &
                & cf_desc, grib2_desc, ldims=shape3d_c )

    cf_desc    = t_cf_var('inv_delta_pres', 'Pa-1', 'inverser of layer thickness at cell center')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'rdelp_c', field%rdelp_c, &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID, &
                & cf_desc, grib2_desc, ldims=shape3d_c )
    ...
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'rdelp_c_new', field%rdelp_c_new, &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,         &
                & cf_desc, grib2_desc, ldims=shape3d_c )

    cf_desc    = t_cf_var('delta_pres_e', 'Pa', 'layer thickness at edge center')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_EDGE)
    CALL add_var( field_list, prefix//'delp_e', field%delp_e,  &
                & GRID_UNSTRUCTURED_EDGE, ZAXIS_HYBRID,&
                & cf_desc, grib2_desc, ldims=shape3d_e )

    cf_desc    = t_cf_var('delta_pres_v', 'Pa', 'layer thickness at vertex center')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_VERTEX)
    CALL add_var( field_list, prefix//'delp_v', field%delp_v,   &
                & GRID_UNSTRUCTURED_VERT, ZAXIS_HYBRID, &
                & cf_desc, grib2_desc, ldims=shape3d_v )

    cf_desc    = t_cf_var('delta_virtual_temp', 'K', 'virtual temperature increment')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'virt_incr', field%virt_incr, &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,     &
                & cf_desc, grib2_desc, ldims=shape3d_c )

    cf_desc    = t_cf_var('virtual_temperature', 'K', 'virtual temperature')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'tempv', field%tempv,    &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,&
                & cf_desc, grib2_desc, ldims=shape3d_c )

    cf_desc    = t_cf_var('rd_log_pr', '', 'Rd * ln(p(k+.5)/p(k-.5)) at cell center')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'rdlnpr_c', field%rdlnpr_c, &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,   &
                & cf_desc, grib2_desc, ldims=shape3d_c )

    cf_desc    = t_cf_var('rd_alpha', '', 'Rd * \alpha at cell center')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
    CALL add_var( field_list, prefix//'rdalpha_c', field%rdalpha_c, &
                & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,     &
                & cf_desc, grib2_desc, ldims=shape3d_c )

    cf_desc    = t_cf_var('mass_flux_edge', 'Pa m s-1', 'mass flux at edge')
    grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_EDGE)
    CALL add_var( field_list, prefix//'mass_flux_e', field%mass_flux_e, &
                & GRID_UNSTRUCTURED_EDGE, ZAXIS_HYBRID,         &
                & cf_desc, grib2_desc, ldims=shape3d_e )
    ...
    IF(ltheta_dyn)THEN
      ...
      grib2_desc = t_grib2_var(255, 255, 255, ibit, GRID_REFERENCE, GRID_CELL)
      CALL add_var( field_list, prefix//'exner', field%exner,    &
                  & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID,&
                  & cf_desc, grib2_desc, ldims=shape3d_c )
    END IF
    ...
    IF (ktracer > 0) THEN
      ...
      CALL add_var( field_list, prefix//'hfl_tracer', field%hfl_tracer,                &
                  & GRID_UNSTRUCTURED_EDGE, ZAXIS_HYBRID,                              &
      ...
      CALL add_var( field_list, prefix//'vfl_tracer', field%vfl_tracer,              &
                  & GRID_UNSTRUCTURED_CELL, ZAXIS_HYBRID_HALF,                       &
      ...
      DO jtrc = 1,ktracer
        ...
        CALL add_ref( field_list, prefix//'hfl_tracer',                                     &
                    & prefix//'hfl_tracer_'//ctracer_list(jtrc:jtrc),                       &
        ...
        CALL add_ref( field_list, prefix//'vfl_tracer',                                     &
                    & prefix//'vfl_tracer_'//ctracer_list(jtrc:jtrc),                       &
