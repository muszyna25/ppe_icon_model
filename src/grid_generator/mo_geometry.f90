...
CONTAINS
  ...
  SUBROUTINE generate_geometry(lplane)
    ...
    SUBROUTINE compute_geometry (klev)
      ...
      DO j=0, novs-1
        ptr_tl%vs(j)%area_dual_cell   = re*re*ptr_tl%vs(j)%area_dual_cell
      ENDDO
      DO j=0,nots-1
        ptr_tl%ts(j)%triangle_area    = re*re*ptr_tl%ts(j)%triangle_area
      ENDDO
      DO j=0, noeds-1
        ptr_tl%es(j)%edge_dual_arc    = re*   ptr_tl%es(j)%edge_dual_arc
        ptr_tl%es(j)%edge_primal_arc  = re*   ptr_tl%es(j)%edge_primal_arc
        ptr_tl%es(j)%edge_vert0_arc   = re*   ptr_tl%es(j)%edge_vert0_arc
        ptr_tl%es(j)%edge_vert1_arc   = re*   ptr_tl%es(j)%edge_vert1_arc
        ptr_tl%es(j)%edge_cell0_arc   = re*   ptr_tl%es(j)%edge_cell0_arc
        ptr_tl%es(j)%edge_cell1_arc   = re*   ptr_tl%es(j)%edge_cell1_arc
      ENDDO
