...
CONTAINS
...
#ifdef DEBUG
     WRITE (nerr, FORMAT_VALS5)routine,' p_pe=',p_pe,' open netCDF file with ID=',me%cdiIds%fHndl
#endif
  ...
  SUBROUTINE restartFile_writeLevel_r(me, varId, levelId, data_out)
    ...
    CALL streamWriteVarSlice(me%cdiIds%fHndl, varId, levelId, data_out, 0)
  ...
  SUBROUTINE restartFile_writeLevel_s(me, varId, levelId, data_out)
    ...
    IF(timers_level >= 7) CALL timer_start(timer_write_restart_io)
    CALL streamWriteVarSliceF(me%cdiIds%fHndl, varId, levelId, data_out, 0)
    IF(timers_level >= 7) CALL timer_stop(timer_write_restart_io)
...
#ifdef DEBUG
      WRITE (nerr,FORMAT_VALS3)routine,' p_pe=',p_pe
      IF (me%cdiIds%fHndl /= CDI_UNDEFID) THEN
        WRITE (nerr,'(3a)')routine,' try to close restart file=',TRIM(me%filename)
        WRITE (nerr, FORMAT_VALS5)routine,' p_pe=',p_pe,' close netCDF file with ID=',me%cdiIds%fHndl
      ENDIF
#endif
