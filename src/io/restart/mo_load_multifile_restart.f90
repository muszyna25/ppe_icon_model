...
MODULE mo_load_multifile_restart
  ...
  CONTAINS
    ...
    PROCEDURE :: redistribute => readBuffer_redistribute
  ...
  TYPE t_MultifilePatchReader
    INTEGER :: domain
    TYPE(t_ReadBuffer), PRIVATE :: readBufferCells, readBufferEdges, readBufferVerts
...
CONTAINS
  ...
  SUBROUTINE payloadFile_readVarLevel(me, varData, level, buffer)
    ...
    INTEGER :: varId, pointCount, trash, windowEnd
    REAL(KIND=dp), POINTER :: pdp(:)
    ...
    IF (pointCount > 0) THEN
      ...
      windowEnd = buffer%curWindowOffset + pointCount - 1
      IF(windowEnd > SIZE(buffer%readBuffer_1d_d)) THEN
        WRITE (0,*) "windowEnd = ", windowEnd, "; SIZE(buffer%readBuffer_1d_d) = ", SIZE(buffer%readBuffer_1d_d)
        CALL finish(routine, "assertion failed: attempt to overfill buffer")
      END IF
      psp  => buffer%readBuffer_1d_s(buffer%curWindowOffset:windowEnd)
      pdp  => buffer%readBuffer_1d_d(buffer%curWindowOffset:windowEnd)
      pint => buffer%readBuffer_1d_int(buffer%curWindowOffset:windowEnd)
      ! actually READ the DATA
      IF(timers_level >= 7) CALL timer_start(timer_load_restart_io)
      SELECT CASE(varData%info%data_type)
      CASE(REAL_T)
        IF(.NOT.ASSOCIATED(pdp)) CALL finish(routine, "assertion failed: buffer not associated")
        IF(pointCount /= SIZE(pdp)) THEN
          CALL finish(routine, "assertion failed: supplied buffer has wrong size")
        END IF
        CALL streamReadVarSlice(me%streamId, varId, level, pdp, trash)
      CASE(SINGLE_T)
        IF(.NOT.ASSOCIATED(psp)) CALL finish(routine, "assertion failed: buffer not associated")
        IF(pointCount /= SIZE(psp)) THEN
          CALL finish(routine, "assertion failed: supplied buffer has wrong size")
        END IF
        CALL streamReadVarSliceF(me%streamId, varId, level, psp, trash)
      CASE(INT_T)
        IF(.NOT.ASSOCIATED(pint)) CALL finish(routine, "assertion failed: buffer not associated")
        IF(pointCount /= SIZE(pint)) THEN
          CALL finish(routine, "assertion failed: supplied buffer has wrong size")
        END IF
      ...
      END SELECT
      IF(timers_level >= 7) CALL timer_stop(timer_load_restart_io)
      buffer%curWindowOffset = windowEnd + 1
    END IF
  ...
  SUBROUTINE readBuffer_redistribute(me, dest_d, dest_s, dest_i)
    CLASS(t_ReadBuffer), INTENT(INOUT) :: me
    REAL(dp), INTENT(INOUT), OPTIONAL :: dest_d(:,:)
    REAL(sp), INTENT(INOUT), OPTIONAL :: dest_s(:,:)
    INTEGER,  INTENT(INOUT), OPTIONAL :: dest_i(:,:)

    IF(timers_level >= 7) CALL timer_start(timer_load_restart_communication)
    IF(PRESENT(dest_s)) &
      CALL exchange_data_noblk(me%commPattern, dest_s, me%readBuffer_1d_s)
    IF(PRESENT(dest_d)) &
      CALL exchange_data_noblk(me%commPattern, dest_d, me%readBuffer_1d_d)
    IF(PRESENT(dest_i)) &
      CALL exchange_data_noblk(me%commPattern, dest_i, me%readBuffer_1d_int)
  ...
  SUBROUTINE multifilePatchReader_readData(me, varData)
    ...
    DO varIndex = 1, SIZE(varData)
      ...
      DO level = 1, levelCount
        ...
        SELECT CASE(varData(varIndex)%info%data_type)
        CASE(REAL_T)
          CALL readBuffer%redistribute(dest_d=levelPointers_d(level)%p)
        CASE(SINGLE_T)
          CALL readBuffer%redistribute(dest_s=levelPointers_s(level)%p)
        CASE(INT_T)
          CALL readBuffer%redistribute(dest_i=levelPointers_int(level)%p)
        CASE DEFAULT
          CALL finish(routine, "Internal error! Variable "//TRIM(varData(varIndex)%info%name))
        END SELECT
      END DO
