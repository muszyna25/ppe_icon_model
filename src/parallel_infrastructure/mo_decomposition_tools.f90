...
!CDIR COLLAPSE
  ...
  SUBROUTINE generate_glb_to_loc_lookup(n, n_g, n_inner, decomp_info)
    ...
    ALLOCATE(decomp_info%inner_glb_index(n_inner), &
      ...
      &      decomp_info%outer_glb_index(n-n_inner), &
      &      decomp_info%outer_glb_index_to_loc(n-n_inner))

    ! Gather inner and outer glb indices
    decomp_info%inner_glb_index(:) = decomp_info%glb_index(1:n_inner)
    decomp_info%inner_glb_index_to_loc(:) = (/(i, i = 1, n_inner)/)
    ...
    decomp_info%outer_glb_index_to_loc(:) = (/(i, i = n_inner+1, n)/)
