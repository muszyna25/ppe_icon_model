...
MODULE mo_opt_nwp_diagnostics
  ...
  PUBLIC :: compute_field_sunshine_duration
...
!$OMP END PARALLEL
  ...
  !! If the direct solar irradiance exeeds 120 Wm-2 the sunshine duration is extended by the fats physics timestep
  !!
  !! @par Revision History
  !! Initial revision by Burkhardt Rockel, Hereon (2021-06-17) 
  !!
  SUBROUTINE compute_field_sunshine_duration( pt_patch,  dt_phy,           &
             &                      sunshine_duration, swflxsfc, swflx_up_sfc, swflx_dn_sfc_diff, cosmu0 )
    ...
    REAL(wp),           INTENT(INOUT) :: sunshine_duration(:,:)   !< sunshine duration (s)
      ...
      CALL get_indices_c( pt_patch, jb, i_startblk, i_endblk,     &
        ...
        DO jc = i_startidx, i_endidx
          ...
          IF ( (swflxsfc(jc,jb) + swflx_up_sfc(jc,jb) - swflx_dn_sfc_diff(jc,jb))/cosmu0(jc,jb)  > 120._wp) THEN
             sunshine_duration(jc,jb) = sunshine_duration(jc,jb) + dt_phy
          END IF
...
!$OMP END PARALLEL
  ...
  END SUBROUTINE compute_field_sunshine_duration
