...
USE mo_communication,       ONLY: exchange_data, exchange_data_grf, exchange_data_grf_old
...
PUBLIC :: interpol_vec_grf, interpol2_vec_grf, interpol_scal_grf ! ,&
          ! interpol_vec_grf_old, interpol2_vec_grf_old, interpol_scal_grf_old
...
!!
SUBROUTINE interpol_vec_grf_old (ptr_pp, ptr_pc, ptr_grf, i_chidx, p_vn_in, p_vn_out)
!
...
IF (my_process_is_mpi_parallel()) THEN
  ...
  CALL exchange_data_grf_old(ptr_pc%comm_pat_interpol_vec_grf,1,nlev_c,nsendtot,nrecvtot, &
                         RECV1=p_vn_out,SEND1=vn_aux,SEND_LBOUND3=LBOUND(vn_aux,3))

ENDIF

END SUBROUTINE interpol_vec_grf_old
...
!!
SUBROUTINE interpol2_vec_grf_old (ptr_pp, ptr_pc, ptr_int, ptr_grf, i_chidx, nfields,&
...
IF (my_process_is_mpi_parallel()) THEN
  ...
  IF (nfields == 1) THEN
    nlevtot = nlev_c
    CALL exchange_data_grf_old(ptr_pc%comm_pat_interpol_vec_grf,nfields,nlevtot,nsendtot, &
...
END SUBROUTINE interpol2_vec_grf_old
...
!!
SUBROUTINE interpol_scal_grf_old (ptr_pp, ptr_pc, ptr_int, ptr_grf, i_chidx, nfields,&
                              f3din1, f3dout1, f3din2, f3dout2, f3din3, f3dout3, &
...
IF (my_process_is_mpi_parallel()) THEN
  ...
  IF (l4d .AND. .NOT. PRESENT(f4din2)) THEN
    ...
    CALL exchange_data_grf_old(ptr_pc%comm_pat_interpol_scal_grf,nfields,nlevtot,nsendtot,       &
...
END SUBROUTINE interpol_scal_grf_old
...
!$OMP DO PRIVATE (jb,jk,je,nlen,nshift,ie,ib) ICON_OMP_DEFAULT_SCHEDULE
    DO jb = 1, nblks_bdyintp
      IF (jb == nblks_bdyintp) THEN
        nlen = npromz_bdyintp
      ELSE
...
!CDIR NODEP
        DO je = nshift+1, nshift+nlen
          ...
          p_vn_out(icheidx(ie,ib,1),jk,icheblk(ie,ib,1))   = vn_aux(jk,je,1)
          p_vn_out(icheidx(ie,ib,2),jk,icheblk(ie,ib,2))   = vn_aux(jk,je,2)

          ! Avoid overwriting prognostic grid points in case of applying this to a prognostic variable
          IF (p_pc%edges%refin_ctrl(icheidx(ie,ib,3),icheblk(ie,ib,3)) <= grf_bdywidth_e) &
            p_vn_out(icheidx(ie,ib,3),jk,icheblk(ie,ib,3))   = vn_aux(jk,je,3)

          IF (p_pp%edges%refin_ctrl(ie,ib) /= -1) THEN
            IF (p_pc%edges%refin_ctrl(icheidx(ie,ib,4),icheblk(ie,ib,4)) <= grf_bdywidth_e) &
            p_vn_out(icheidx(ie,ib,4),jk,icheblk(ie,ib,4)) = vn_aux(jk,je,4)
          ENDIF
...
!$OMP END PARALLEL
  ...
  IF (my_process_is_mpi_parallel()) THEN
    ...
    nrecvtot = SUM(p_pc%comm_pat_interpol_vec_grf(1:4)%n_recv)
    CALL exchange_data_grf(p_pc%comm_pat_interpol_vec_grf,1,nlev_c,nsendtot,nrecvtot, &
...
!CDIR NODEP
          DO je = nshift+1, nshift+nlen
            ...
            p_out(jn)%fld(icheidx(ie,ib,1),jk,icheblk(ie,ib,1))   = vn_aux(jk,je,1,jn)
            p_out(jn)%fld(icheidx(ie,ib,2),jk,icheblk(ie,ib,2))   = vn_aux(jk,je,2,jn)

            ! Avoid overwriting prognostic grid points in case of applying this to a prognostic variable
            IF (p_pc%edges%refin_ctrl(icheidx(ie,ib,3),icheblk(ie,ib,3)) <= grf_bdywidth_e) &
              p_out(jn)%fld(icheidx(ie,ib,3),jk,icheblk(ie,ib,3))   = vn_aux(jk,je,3,jn)

            IF (p_pp%edges%refin_ctrl(ie,ib) /= -1) THEN
              IF (p_pc%edges%refin_ctrl(icheidx(ie,ib,4),icheblk(ie,ib,4)) <= grf_bdywidth_e) &
              p_out(jn)%fld(icheidx(ie,ib,4),jk,icheblk(ie,ib,4)) = vn_aux(jk,je,4,jn)
            ENDIF
...
!$OMP END PARALLEL
  ...
  IF (my_process_is_mpi_parallel()) THEN
    ...
    IF (nfields == 1) THEN
      nlevtot = nlev_c
      CALL exchange_data_grf(p_pc%comm_pat_interpol_vec_grf,nfields,nlevtot,nsendtot, &
                             nrecvtot,RECV1=f3dout1,SEND1=vn_aux(:,:,:,1) )
...
SUBROUTINE interpol_scal_grf (p_pp, p_pc, p_grf, nfields,&
  ...
  ! Auxiliary fields
  REAL(wp), DIMENSION(MAX(32,p_pc%nlevp1),p_grf%npoints_bdyintp_c) :: &
    grad_x, grad_y, maxval_neighb, minval_neighb, val_ctr
  REAL(wp) :: h_aux(MAX(32,p_pc%nlevp1),p_grf%npoints_bdyintp_c,4,nfields)
  REAL(wp) :: limfac1, limfac2, limfac, min_expval, max_expval, epsi, ovsht_fac, r_ovsht_fac, &
              relaxed_minval, relaxed_maxval
...
!$OMP DO PRIVATE (jb,nlen,nshift,jk,jc,ic,ib) ICON_OMP_DEFAULT_SCHEDULE
      DO jb = 1, nblks_bdyintp
        IF (jb == nblks_bdyintp) THEN
          nlen = npromz_bdyintp
        ELSE
...
!CDIR NODEP
          DO jc = nshift+1, nshift+nlen
            ...
            p_out(jn)%fld(ichcidx(ic,ib,1),jk,ichcblk(ic,ib,1)) = h_aux(jk,jc,1,jn)
            p_out(jn)%fld(ichcidx(ic,ib,2),jk,ichcblk(ic,ib,2)) = h_aux(jk,jc,2,jn)
            p_out(jn)%fld(ichcidx(ic,ib,3),jk,ichcblk(ic,ib,3)) = h_aux(jk,jc,3,jn)
            p_out(jn)%fld(ichcidx(ic,ib,4),jk,ichcblk(ic,ib,4)) = h_aux(jk,jc,4,jn)
...
!$OMP END PARALLEL
  ...
  IF (my_process_is_mpi_parallel()) THEN
    ...
    nsendtot = SUM(p_pc%comm_pat_interpol_scal_grf(1:4)%n_send)
    nrecvtot = SUM(p_pc%comm_pat_interpol_scal_grf(1:4)%n_recv)

    IF (l4d .AND. .NOT. PRESENT(f4din2)) THEN

      nlevtot = SIZE(f4dout1,2)*nfields
      CALL exchange_data_grf(p_pc%comm_pat_interpol_scal_grf,nfields,nlevtot,nsendtot,       &
                             nrecvtot,RECV4D1=f4dout1,SEND4D1=h_aux)
