...
MODULE mo_echam_phy_main
  ...
  ! variables that are time-independent, potentailly  shared by the all routines of this module 
...
CONTAINS
  ...
  SUBROUTINE echam_phy_main( patch,                         &
    &                        rl_start, rl_end,              &
    &                        this_datetime,pdtime,inpsteplen, &
    &                        ltrig_rad                      )
    ...
    INTEGER  :: ictop (nproma)             !< from massflux

    REAL(wp) :: zfrc (nproma,nsfc_type,patch%nblks_c)    !< zfrl, zfrw, zfrc combined

    INTEGER  :: ilab   (nproma,nlev)
!    REAL(wp) :: zcvcbot(nproma)
!    REAL(wp) :: zwcape (nproma)

    REAL(wp) :: zqtec  (nproma,nlev)       !< tracer tendency due to entrainment/detrainment
...
!!$    REAL(wp) :: zq_vdf (nproma,nlev)       !< heating by vertical diffusion     [W/m2]
    REAL(wp) :: zq_gwh (nproma,nlev)       !< heating by atm. gravity waves     [W/m2]
!!$    REAL(wp) :: zq_cnv (nproma,nlev)       !< heating by convection             [W/m2]
!!$    REAL(wp) :: zq_cld (nproma,nlev)       !< heating by stratiform clouds      [W/m2]
...
!$OMP PARALLEL DO PRIVATE(jcs,jce)
    DO jb = i_startblk,i_endblk
      CALL get_indices_c(patch, jb,i_startblk,i_endblk, jcs,jce, rl_start, rl_end)
      CALL calculate_zcair_zconv( jb,jcs,jce, nproma, zcd, zcv, field, zcair(:,:,jb), zconv(:,:,jb))
    ENDDO
!$OMP END PARALLEL DO 
...
!$OMP PARALLEL DO PRIVATE(jcs,jce, zq_rsw)
      DO jb = i_startblk,i_endblk
        ...
        CALL echam_radheating( jb,jcs,jce, nproma, field, zq_rsw, zq_rlw(:,:,jb))
...
!$OMP PARALLEL DO PRIVATE(jcs,jce, zxt_emis)
    ...
    !       ENDIF
        ...
        CALL echam_vdiffDown_vdiffUp_surf( jb,jcs,jce, nproma, zxt_emis, field, tend)
...
!$OMP PARALLEL DO PRIVATE(jcs,jce,)
      DO jb = i_startblk,i_endblk
        ...
        CALL echam_gw_hinesg(patch, jb,jcs,jce, nproma, field, tend, zq_phy(:,:,jb))
      ENDDO
!$OMP END PARALLEL DO 
...
!$OMP PARALLEL DO PRIVATE(jcs,jce)
      DO jb = i_startblk,i_endblk
        CALL get_indices_c(patch, jb,i_startblk,i_endblk, jcs,jce, rl_start, rl_end)
        ! this should not be necessary, as filed are initialize to zero
...
!$OMP PARALLEL DO PRIVATE(jcs,jce)
      DO jb = i_startblk,i_endblk
        ...
        CALL echam_ssodrag(patch, jb,jcs,jce, nproma, field, tend, zq_phy(:,:,jb))
      ENDDO
!$OMP END PARALLEL DO 
    ...
    ELSE ! NECESSARY COMPUTATIONS IF SSODRAG IS BY-PASSED
      ...
      ! not necessary, initialized with zeroes
!$OMP PARALLEL DO PRIVATE(jcs,jce)
      DO jb = i_startblk,i_endblk
        CALL get_indices_c(patch, jb,i_startblk,i_endblk, jcs,jce, rl_start, rl_end)
...
!$OMP END PARALLEL DO 
    ...
    IF (phy_config%lconv) THEN
      ...
      CALL cucall( jce, nproma, nlev,          &! in
        &          nlevp1, nlevm1,            &! in
        &          ntrac,                     &! in     tracers
!        &          jb,                        &! in     row index
        &          psteplen,                  &! in
...
!0      CALL cloud_cdnc_icnc(...) !!skipped in ICON
  ...
  SUBROUTINE calculate_zcair_zconv( jb,jcs,jce, nbdim, field, zcair, zconv)
    ...
    INTEGER         ,INTENT(IN) :: jcs, jce       !< start/end column index within this block
    INTEGER         ,INTENT(IN) :: nbdim          !< size of this block
    REAL(wp)         ,INTENT(IN) :: zcd                       !< specific heat of dry air          [J/K/kg]
    REAL(wp)         ,INTENT(IN) :: zcv                       !< specific heat of water vapor      [J/K/kg]
  ...
  !---------------------------------------------------------------------
  SUBROUTINE echam_radheating( jb,jcs,jce, nbdim, field, zq_rsw, zq_rlw)
    INTEGER         ,INTENT(IN) :: jb             !< block index
    INTEGER         ,INTENT(IN) :: jcs, jce       !< start/end column index within this block
    INTEGER         ,INTENT(IN) :: nbdim          !< size of this block
    TYPE(t_echam_phy_field),   POINTER :: field    ! in
  ...
  SUBROUTINE echam_vdiffDown_vdiffUp_surf( jb,jcs,jce, nbdim, zxt_emis, field, tend)
    INTEGER         ,INTENT(IN) :: jb             !< block index
    INTEGER         ,INTENT(IN) :: jcs, jce       !< start/end column index within this block
    INTEGER         ,INTENT(IN) :: nbdim          !< size of this block
...
!    REAL(wp) :: zcfnc_tile(nbdim,nsfc_type)  !<  for "nsurf_diag"
    ...
    REAL(wp) :: zvmixtau   (nproma,nlev)   !< timescale of mixing for vertical turbulence
      ...
      CALL vdiff_down( vdiff_config%lsfc_mom_flux,      &! in
                     ...
                     & zfrc(:,:,jb),                    &! in, area fraction of each sfc type
...
!397 END DO
  ...
  END SUBROUTINE echam_vdiffDown_vdiffUp_surf
  !---------------------------------------------------------------------
...
!$OMP PARALLEL DO PRIVATE(jcs,jce,do3dt)
    DO jb = i_startblk,i_endblk
      CALL get_indices_c(patch, jb,i_startblk,i_endblk, jcs,jce, rl_start, rl_end)
...
!$OMP END PARALLEL DO 
  ...
  !---------------------------------------------------------------------
  SUBROUTINE echam_gw_hinesg(patch, jb,jcs,jce, nbdim,  field, tend, zq_phy)
    TYPE(t_patch)   ,INTENT(in), TARGET :: patch           !< grid/patch info
    INTEGER         ,INTENT(IN) :: jb             !< block index
    INTEGER         ,INTENT(IN) :: jcs, jce       !< start/end column index within this block
    INTEGER         ,INTENT(IN) :: nbdim          !< size of this block  !---------------------------------------------------------------------
...
!!$        &             aprflux(:,krow)          ,&
    ...
    ! tendency
    tend% ta_gwh(jcs:jce,:,jb) = zq_gwh(jcs:jce,:)*zconv(jcs:jce,:,jb)
  ...
  SUBROUTINE echam_gw_hinesg(patch, jb,jcs,jce, nbdim,  field, tend)
  !---------------------------------------------------------------------

  !---------------------------------------------------------------------
  SUBROUTINE echam_ssodrag(patch, jb,jcs,jce, nbdim, field, tend, zq_phy)
    TYPE(t_patch)   ,INTENT(in), TARGET :: patch           !< grid/patch info
    INTEGER         ,INTENT(IN) :: jb             !< block index
    INTEGER         ,INTENT(IN) :: jcs, jce       !< start/end column index within this block
    INTEGER         ,INTENT(IN) :: nbdim          !< size of this block  !---------------------------------------------------------------------
    ...
    CALL ssodrag( nc                                        ,& ! in,  number of cells/columns in loop (jce-jcs+1)
      ...
      ! tendency
      tend% ta_sso(jcs:jce,:,jb) = zq_sso(jcs:jce,:)*zconv(jcs:jce,:,jb)
