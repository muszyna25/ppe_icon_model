...
SUBROUTINE gwstress( kbdim,  klev,                  &
     &               kgwd,   kdx,    kkenvh,        &
     &               pstd,   psig,   ppic,   pval,  &
...
!IBM* ASSERT(NODEPS)
  DO 301 ji=1,kgwd
     ...
     ptau0(jl) = gkdrag*prho(jl,klev+1)                  &
...
2110 END DO
  ...
  DO 301 jl=1,kproma
     IF(ktest(jl) == 1) THEN
        ztau(jl,klev+1)= - gklift*zrho(jl,klev+1)*pcoriol(jl)  &
            !&          *(2._wp*pstd(jl)+pmea(jl))             &
             &          * 2._wp*pstd(jl)                       &
             &          * pvlow(jl)
        ztav(jl,klev+1)=   gklift*zrho(jl,klev+1)*pcoriol(jl)  &
            !&          *(2._wp*pstd(jl)+pmea(jl))             &
             &          * 2._wp*pstd(jl)                       &
             &          * pulow(jl)
     ELSE
