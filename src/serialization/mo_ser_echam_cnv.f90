...
#if defined(SERIALIZE_ECHAM_CNV) || defined(SERIALIZE_ECHAM_ALL) || defined(SERIALIZE_ALL)
...
#error SERIALIZATION MODE IS NOT SET
    ...
    !$ser data echam_cnv_pfull=field%pfull(:,:,jb) IF (ASSOCIATED(field%pfull))
    !$ser data echam_cnv_phalf=field%phalf(:,:,jb) IF (ASSOCIATED(field%phalf))
