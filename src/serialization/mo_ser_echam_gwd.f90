...
#if defined(SERIALIZE_ECHAM_GWD) || defined(SERIALIZE_ECHAM_ALL) || defined(SERIALIZE_ALL)
...
#endif
    !$ser data echam_gwd_phalf=field%phalf(:,:,jb) IF (ASSOCIATED(field%phalf))
    !$ser data echam_gwd_pfull=field%pfull(:,:,jb) IF (ASSOCIATED(field%pfull))
