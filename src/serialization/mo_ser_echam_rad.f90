...
MODULE mo_ser_echam_rad
  ...
  SUBROUTINE serialize_rad_input(jg, nproma, nlev, field)
    ...
    !$ser verbatim   !NOACC UPDATE HOST( field%phalf ) IF( ASSOCIATED( field%phalf )) 
    !$ser verbatim   !NOACC UPDATE HOST( field%pfull ) IF( ASSOCIATED( field%pfull )) 
...
#error SERIALIZATION MODE IS NOT SET
    ...
    !$ser data echam_rad_phalf=field%phalf(:,:,:) IF (ASSOCIATED(field%phalf)) 
    !$ser data echam_rad_pfull=field%pfull(:,:,:) IF (ASSOCIATED(field%pfull)) 
    ...
    !$ser verbatim   !NOACC UPDATE DEVICE( field%phalf ) IF( ASSOCIATED( field%phalf )) 
    !$ser verbatim   !NOACC UPDATE DEVICE( field%pfull ) IF( ASSOCIATED( field%pfull )) 
