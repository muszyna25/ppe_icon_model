...
# output
             ...
             TARGET=${TARGET:-phc}
      if [ "$TARGET" = 'phc' ]; then
        TARGET_FILE='ts_phc3.0_annual'
                LEV=${LEV:-L40}
         _test_file="${PHC_DIR}/PHC__3.0__TempO__1x1__annual.nc"
      else
        TARGET_FILE='omip'
                LEV=${LEV:-forcing}
...
targetWeight=./cell_weight-${remapOperator}-${TARGET}-to-${MODEL}-${GRID}.nc
#==============================================================================
# MAIN
#==============================================================================
...
if test ! -f "${targetWeight}" -o "$FORCE" -eq 1 ;then
  ...
  $CDO -P ${THREADS} ${remapOperator},${targetGrid} ${_test_file} ${targetWeight}
fi
#==============================================================================
case "${TARGET}" in
  phc*)
    ...
    #==============================================================================
    # tmpfiles
    TMP='_phc3.0-annual.nc'
    MERGED='phc3.0-annual.nc'
    ...
    cd ${PHC_DIR}
    $CDO -O merge ${PHC_FILES} $OLDPWD/${TMP}
    cd -
    $CDO -adipot -setcode,-1 -chname,SO,s,TempO,t ${TMP} ${MERGED}

    #==============================================================================
    # filling of land points
    $CDODEV -P ${THREADS} -O -r -settaxis,2000-01-01,0,years -fillmiss1s -ifthenelse -setmisstoc,0 ${MERGED} ${MERGED}  -remapbil,${MERGED} -fillmiss1s,4  -sellonlatbox,20,28,64,66 ${MERGED} ${NOMISS}

    $CDO -remap,${targetGrid},${targetWeight} ${NOMISS} ${TARGET_MODEL_NOMISS}
  ...
  omip)
    echo "Running in 'omip' mode"

    # create the correct lsm
    lsmFile='lsm4omip.nc'
    ...
    for file in $(ls ${OMIP_DIR}/${OMIP_FILES}); do 
      ...
      $CDO fillmiss -ifthen ${lsmFile}  $file ${_oFile}
