...
# cat, yearmean, mulc, netcdf of MOC to moc.ym.nc
files=$(echo $mocPattern | wc -w)
if [[ $files -gt 1 ]]; then
  cdo -f nc -r -mulc,1.e-9 -yearmean -cat "${mocPattern}"  moc.ym.nc
...
if [[ "$mocPattern" != "moc.dat" ]]; then
  ...
  if [[ "20" = $(cdo nlevel -selvar,$mocVar $(ls -1 $mocPattern | head -n 1)) ]]; then
    cdo outputkey,date,value -fldmean -selname,$mocVar -sellonlatbox,0,1,40,60 \
         -sellevel,1000 -setgrid,r1x180 moc.ym.nc > moc.dat
  else
    cdo outputkey,date,value -fldmean -selname,$mocVar -sellonlatbox,0,1,40,60 \
         -intlevel,1000 -sellevel,900/1100 -setgrid,r1x180 moc.ym.nc > moc.dat
  fi
fi
